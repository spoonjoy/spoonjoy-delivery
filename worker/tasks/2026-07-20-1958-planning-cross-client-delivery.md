...
- [ ] A mandatory rebaseline bundle records fetched web/native `main` SHAs, active release-task commit, signed owner-release handoff, matching receiver acknowledgment, zero in-flight source mutations/deploys, and terminal cleanup ownership; source edits and the pilot remain blocked until its validator passes.
- [ ] A Candidate Authorization binds Product Change ID, pack digest, exact source SHA(s), monotonically increasing lock generation, expiry, and permitted provider mutation. Production deploy and TestFlight workflows reject absent, stale, superseded, wrong-generation, or wrong-SHA authorization, including legacy unbound dispatches.
...
- Gate provider mutations with a Candidate Authorization that binds Product Change, pack digest, exact source SHA(s), lock generation, expiry, and allowed mutation. Existing production/TestFlight workflows must reject unbound or stale authorization before this process becomes shipment authority.

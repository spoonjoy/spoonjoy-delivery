...
**What**: Implement `src/authority.ts`, `src/ledger.ts`, `src/transitions.ts`, immutable actor policy `policies/authority-v1.yaml`, exact source-agnostic authority rules for `github-environment-ui-bootstrap-v1` and `github-environment-ui-recovery-v1`, append payload/digest generation, and local history validation for `refs/heads/release-ledger`. Bootstrap success is consumed once per repository/environment identity; a pre-mutation abort may be reauthorized; any partial mutation must remain under the same active claim until authoritative UI/API proof shows either the exact original state or the exact protected target state. Recovery is permitted only on proven live drift and may restore only the last terminally attested protected policy.
**Output**: Deterministic ledger transition engine.
**Acceptance**: Only protected-workflow/provider-bound actors and legal expected-parent transitions validate; bootstrap cannot be consumed twice, pre-mutation abort does not consume it, partial state cannot terminate ambiguously, recovery cannot alter policy or run without drift, and neither exception authorizes source/provider mutation; shipment is possible only through direct-child `ReleaseSetPublished`.
...
**Acceptance**: Receipt binds exact claim/graph/actor/repository/environment and exception version; exact target success consumes the sole bootstrap, exact-prestate pre-mutation abort does not, and partial/ambiguous state cannot terminate; UI and API evidence agree on every API-visible field; UI independently proves administrator bypass is off; no workflow or deploy runs.
...
**Output**: Authorization/claim/success-or-pre-mutation-abort commit; per-attempt actor-viewer response; exact repository/environment IDs; before/after REST responses; before/apply/after screenshot, accessibility-tree, and URL digests; `operation_execution_kind=github_ui_bootstrap_v1`; explicitly null workflow run/attempt fields; governance receipt digest; and authoritative post-query.
**Acceptance**: Receipt binds exact claim/graph/actor/repository/environment and exception version; exact target success consumes the sole bootstrap, exact-prestate pre-mutation abort does not, and partial/ambiguous state cannot terminate; UI and API evidence agree on every API-visible field; UI independently proves administrator bypass is off; no workflow or TestFlight run starts.
...
### ⬜ Unit 63a: Last-Moment Attestor Refresh
**What**: When Unit 63 is green, use authenticated local `gh` to dispatch the exact merged source-owned read-only web/native attestors, wait for terminal artifacts, verify their workflow/source/delivery identities and GitHub attestations, replace the Release Set's freshness-bound evidence, recompile, and rerun the complete leak scan. Record the attestor completion timestamps and the maximum 120-second interval in which Unit 64 may start. If Unit 63 is not green or any refresh fails, emit a structured failure report without publication.
**Output**: Final attestor run/attempt/artifact/attestation IDs, refreshed canonical Release Set digest, provider query timestamps, 120-second append-start deadline, and green or structured failure report.
**Acceptance**: All private provider evidence comes from source-owned attestors at exact source/delivery SHAs; the refreshed set is fully revalidated and leak-free; no provider mutation occurs; failure routes to Unit 63b rather than publication.
...
**What**: After Unit 63b records a green publish decision, start the protected append workflow within Unit 63a's 120-second interval. Before choosing a terminal, the workflow re-queries source main/check identities, source mutation-run inventories since each attestor timestamp, public Worker/runtime digests, evidence expiry, exact attestor artifact identities, and ledger parent. If every value is unchanged, expected-parent append the complete Release Set as the direct child of Unit 61. If any value changed, expired, or cannot be queried, append `FinalizationAborted` instead and return to reviewer-gated repair/replay at Unit 60a.
**Output**: Append run ID, last-moment query digest, ledger parent, authoritative `ReleaseSetPublished` or `FinalizationAborted` commit, actor/workflow proof, and post-query.
**Acceptance**: Unit 63b decision, Unit 63a deadline/artifacts, and Unit 61 parent are exact; the workflow emits exactly one direct-child terminal; only zero drift can produce the sole shipment event, while drift atomically aborts the generation before repair.
...
- 2026-07-20 23:19: Validation Round 4 defined non-consuming pre-mutation abort and same-claim partial bootstrap recovery, added restore-only live-drift governance recovery, and required last-moment source attestors plus an in-workflow zero-drift query before publication.

...
**What**: Implement `src/operation-graph.ts`, `src/receipts.ts`, deterministic rollback-target selection/setup planning, fixed source-agnostic governance, repository-distinct supervisor-envelope start/provisional-challenge/local-zero/provision-or-abort/rotate/retire graphs, an inert keyless App identity-registration graph, a post-root delivery-governance/broker-upgrade graph, canonical expression resolution, dry-run/revalidation/receipt/containment, and `ledger-root-bootstrap-v1`. Key graphs bind Cloudflare secret prestate, provisional/active event generation, canonical X25519 key/fingerprint, exact Started commit, byte-identical repository/generation/key/fingerprint plus challenge/local-zero receipts in Provisioned, exact challenge/ordinary OIDC allowlists and non-consuming wrong-kind branches, active-run retention, bootstrap credential, delete/contain alternatives, and terminal. Identity registration permits only metadata-only/no-webhook/no-key/no-bypass App creation and installation, records GitHub-returned IDs, and on ambiguity either proves that exact inert identity or removes it before retry. Post-root upgrade binds exact root event and live settings/broker prestate, permits only reviewed deltas/rollback/disable containment, and forbids root/App/genesis changes. The root graph binds authenticated operator, exact pre-registered App/install/repository IDs and repository/broker/ruleset/environment prestate plus code digests; permits only install, exact-target resume, or exact-prestate rollback; emits one App-authenticated `RootAuthorityInstalled`; verifies setup credential/path removal; and thereafter rejects every root-bootstrap invocation. Candidate selection retains documented total-order/fail-closed rules.
...
**Acceptance**: The exact reviewed tree proves metadata-only/no-webhook/no-key/no-OAuth/no-device-flow/no-user-authorization/no-bypass targets and total ambiguous-response cleanup; all gates are green at 100% coverage with zero warnings; no App/provider mutation has occurred.

### ⬜ Unit 11c1: Inert Ledger App Identity Registration
**What**: Consume Unit 11c0's exact no-mutation envelope and use only the authenticated operator plus that reviewed identity-registration DAG to create/install exactly one delivery-ledger GitHub App with metadata-only permissions, no webhook, no private key, no enabled OAuth callback/device flow/user authorization, and no repository bypass. Stream any provider-issued client secret/private material directly to destruction without arguments/logs/artifacts/persistent plaintext, delete/revoke its provider-side credential records, and prove local-zero plus empty key/client-secret inventory. Record provider-returned App, installation, repository, owner, manifest, permission, and creation identities. On lost response or duplicate state, re-query by immutable owner/slug/manifest and either prove one exact inert identity or uninstall/delete every attempt before retry; no usable key/token, broker secret, rule, ref, environment, or ledger mutation is allowed.
**Output**: Exact reviewed-head/envelope binding; provider request/response/post-query receipts; exact stable App/installation/repository IDs; OAuth/device-flow/user-authorization/private-material absence, provider-side credential deletion, empty inventory, and local-zero proof; inert-capability proof; duplicate/ambiguity reconciliation; and delete-or-retain terminal.
**Acceptance**: Exactly one keyless, tokenless, metadata-only, no-webhook, no-OAuth/device-flow/user-authorization, no-bypass App installation exists and cannot authenticate or write; every provider-issued secret and ambiguous/extra identity is absent; returned IDs are now available for policy/intent generation.

### ⬜ Unit 11d: Root Bootstrap Intent and Dry Run
**Prerequisite**: Re-query Unit 11c1's exact inert App/installation/repository identity and fail if it has any usable key/token/client secret, webhook, OAuth callback, device flow, user authorization, non-metadata permission, bypass, or identity drift. Bind those provider-returned IDs and inert manifest digest into the immutable authority policy, root intent, existing-App key target, and every later App assertion before review/signing.
...
**Output**: Canonical unsigned acyclic intent; exact inert App identity receipt/IDs; inline manifest and policy/schema projections; exact canonical-byte/digest/key/SPKI/signature golden vectors; detached signature proof; exact prestates/targets/genesis/events; journal ref/genesis/commit schemas; full crash/reconcile table; and zero-additional-mutation proof.
**Acceptance**: Independent implementations reproduce every byte and digest including provider-returned App identities, reject excluded/self-referential/aliased/nonregular inputs, and verify the exact P-256/SHA-256 signature domain. Intent content is committed/reviewable and contains no self-reference; every step has exact pre/post/query/reconcile semantics; the Unit 11c1 identity remains inert and no additional live setting, ref, App, broker, environment, or provider mutation occurs.
...
**Authority binding**: The reviewed head, immutable policy, root intent, and `RootBootstrapEnvelopeV1` must all carry the exact provider-returned Unit 11c1 App/installation/repository IDs and inert manifest digest; any identity/capability drift invalidates the envelope and returns to review.
...
**What**: Without touching source/provider state, treat repairs `2ad767db1a3237b12dcda45e0f398119e8d3e8f8`, `f324901fc246bf9743503ab5aa46f59f87ff230a`, and `7411307777e9828cea5dc1b33d62434ab4ea2fdd` as nonterminal; the last fixes App substitution but still publishes absolute worktree paths and permits Integration-App bypass on protected mains. Require a later explicitly reviewed, remote-reachable upstream successor. `ProtectedMainV1` consumes repository-specific allowlists of exact `{context, expected App ID}` required checks, normalizes every classic/ruleset layer, requires active PR/strict checks/no force-delete and an empty bypass/allowance set across users, teams, roles, admins, and Integration Apps, and verifies stable digests; wrong positive App IDs/spoof checks fail. `ProtectedLedgerV1` derives the dedicated ledger App ID only from exact-byte protected delivery authority policy and `RootAuthorityInstalled` artifacts whose ref/commit/path/SHA-256 and mutual policy/App binding are re-queried, treats source/acknowledgment/event App IDs as redundant equal echoes, requires active mutability blocking and exactly that anchored App bypass, and permits no user/team/role/admin/other-App/direct writer path. Public outbound/ack bytes use only random stable worktree IDs/roles and recursively reject local/private paths plus unkeyed path-derived digests. Negative tests substitute acknowledgment/event/ledger bypass together; add any classic App allowance or ruleset Integration bypass to either main; and inject absolute paths or path hashes. Verify canonical handoff schemas/checker, acyclic ledger binding, both mains, ledger, and exact ack bytes.
...
**Acceptance**: Remote committed successor is independently green before Unit 13a1; both mains satisfy exact expected-source policy with zero bypass while ledger has only the exact App bypass; wrong/missing/extra-positive source, any main bypass, wrong/extra ledger bypass, permissive/drifting rules, branch-only evidence, and self-reference fail; upstream still owns source; no source/provider mutation by this task.
...
**What**: Provision the existing role-based ASC key into the disabled broker through a one-use scoped Cloudflare secret-import operation with inventory/challenge/local-zero/credential-revocation proof, then remove or prove absence of every source Actions copy before activation. The claim-bound protected native workflow presents exact OIDC plus a one-use query manifest; broker validates identity/claim/JTI, builds only allowlisted GET requests itself, follows only allowlisted pagination links, performs ASC calls, strips raw tester/user/token fields, signs the bounded sanitized response, consumes the nonce, and returns no credential or generic lease. Process that response under the Unit 22 macOS supervisor/profile; its child has a unique UID, only manifested writable roots, and no ASC/signer/unwrap/lease capability. Terminate/re-enumerate the UID before root destruction/fsync and pre-bound evidence signing. Separate no-secret job rejects alternate keys, verifies broker signature/evidence/teardown, uploads allowlisted artifacts, and inventories GitHub objects. Cancellation, request drift, broker uncertainty, out-of-root write, unsupported isolation, or absent teardown aborts or contains the ASC broker capability.
**Output**: ASC key custody/import/removal/inventory receipts; exact query manifest and broker request/response/signature receipts; independent sanitized ASC evidence; and separately verified sandbox teardown attestation.
**Acceptance**: Fixture and live read-only run pass with captured outgoing GETs and zero ASC mutation; no Actions job or child ever receives the role-based ASC key, JWT, generic HTTP capability, raw tester response, or self-attestation key; no credential/cache residue or unclassified object remains.

### ⬜ Unit 23c: Native ASC Attestor - Coverage
**What**: Cover ASC broker identity/JTI/nonce, exact outgoing method/host/path/query/header construction, pagination allowlist, SSRF/mutation/caller-override denial, response schema/size/redaction/signature, key import/inventory/source-copy removal/local-zero/revocation/containment, plus macOS bootstrap binding, child credential/signer/unwrap/lease denial, deny-write probes, detached descendants/UID exhaustion, alternate-key rejection, teardown, separate verifier, artifact/cache, cancellation, unsupported boundary, and absent-disposal branches.

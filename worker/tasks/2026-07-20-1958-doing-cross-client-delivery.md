...
**Release-tag authority**: Red tests must model `ProtectedReleaseTagV1` over `refs/tags/release-set/**`, exactly one Ledger Writer Integration bypass, creation/update/deletion/non-fast-forward rules, canonical create-if-absent tag events, wrong principal/pattern/object, collision/lost-response reconciliation, protection drift, and absolute denial of update/delete broker methods.
...
**Release-tag graph**: Add fictional-provider red coverage for a root-bound namespace-ruleset install/reconcile graph and a separately claimed `release-tag-project-v1` create-if-absent graph, including every collision, response-loss, stale-ruleset, alternate principal/object, and forbidden update/delete path.
...
**Release-tag graph**: Implement root-install and projection graphs that bind the stable active `ProtectedReleaseTagV1` digest before/after every query, allow only Ledger Writer broker creation of one canonical annotated tag/ref from an exact observed publication, and reconcile exact equality on retry; mismatched existing refs/releases contain and no graph can update/delete a tag.
...
**Release-tag authority**: Normalize/verify active tag rulesets independently from branch/ledger rules. `ProtectedReleaseTagV1` matches only `refs/tags/release-set/**`, requires creation/update/deletion/non-fast-forward restrictions, exactly one always-bypass Ledger Writer Integration, and stable digests. Its broker validates `release-tag-project-v1`, creates exact annotated object/ref only when absent, and exposes no update/delete/force/general-tag endpoint.
...
**Release-tag authority**: Cover wrong/missing/extra tag rules, pattern aliases, wrong or extra bypass actor/mode, inactive/evaluate layers, stable-digest drift, ordinary-token/App/user creation, alternate object/name, existing-ref equality/mismatch, response loss, rerun, and every attempted update/delete/force route.
...
**What**: Freeze the exact remote-reachable delivery commit containing the distinct Ledger Writer and Delivery Guardian identity-registration DAGs and complete captured-request/crash/reconciliation tests. Generate a distinct non-exportable P-256 reviewer-attestation key in Secure Enclave under an access-control tag separate from root/App/envelope keys; export only its RFC 5480 SPKI bytes/fingerprint/generation, challenge-sign the exact domain, and record local inventory. Dispatch two distinct fresh hostile reviewer tasks with frozen prompt/lens and target digests. Canonicalize `ReviewerEnvelopeV1`, sign it through the local key service, and verify signature, target, task/model/verdict/evidence, nonce, expiry, and local one-use consumption before any provider mutation. Run full local gates, repair every BLOCKER/MAJOR, and repeat until the exact tree is clean.
**Output**: Exact commit/tree, full gate logs, two fresh reviewer task/model/verdict/evidence digests, reviewer-key SPKI/fingerprint/generation/challenge and workflow-inaccessibility proof, repair commits, canonical signed no-mutation envelope/digest/nonce/expiry, one-use receipt, and clean provider pre-query.
**Acceptance**: The exact reviewed tree proves two distinct metadata-only/no-webhook/no-key/no-OAuth/no-device-flow/no-user-authorization/no-bypass targets, repository-install separation, total ambiguous-response cleanup, and a workflow-inaccessible non-exportable reviewer signer whose canonical envelope cannot be forged, replayed, retargeted, or used after expiry/revocation. All gates are green at 100% coverage with zero warnings; no App/provider mutation has occurred; V1 records reviewer integrity/freshness without claiming separation from the sole operator.
...
**Prerequisite**: Re-query Unit 11c1's exact distinct inert Ledger Writer and Delivery Guardian App/installation/repository identities and Unit 11c0's exact reviewer SPKI/fingerprint/generation; fail if either App has any usable key/token/client secret, webhook, OAuth callback, device flow, user authorization, non-metadata permission, bypass, source installation, or identity drift, or if the reviewer key is exportable/unavailable/substituted. Bind both provider identity sets, reviewer signer, inert manifest digests, target permissions/installations, source-cutover targets, and non-substitution invariant into the immutable authority policy, root intent, existing-App key targets, and every later assertion before review/signing.
**Tag-governance binding**: Root intent must bind exact `ProtectedReleaseTagV1` name/target/include/exclude/rule/bypass digests, Ledger Writer App ID, `release-tag-project-v1` create-only broker contract, canonical tag schema/domain, and rollback/removal behavior before any live rule exists.
...
**What**: Open the delivery PR, require zero GitHub approving reviews, create one fresh review request nonce, dispatch two distinct hostile reviewer tasks against the exact head/tree/workflow with frozen prompt/lens digests, and collect their evidence through canonical `ReviewerEnvelopeV1`. Sign only after both verdicts are `CLEAN`; verify the root-intent-bound reviewer SPKI/generation, signature domain, task/model/evidence identities, target, issuance/expiry/revocation, and atomically consume the nonce in local pre-root state. Run exact-head CI and repair PR-only failures, invalidating every prior envelope on head/tree/workflow change. After final head/review is frozen, create a protected GitHub artifact attestation `RootBootstrapEnvelopeV1` binding exact head, unsigned intent digest, `rootCodeManifest`/root-code/policy-projection/schema-projection digests, pinned root and reviewer SPKI fingerprints, reviewer envelope/consumption receipt, detached-signature digest and verification result, expected current-main SHA, exact workflow provenance, CI/reviewer verdicts, actor, issuance/expiry, and no-mutation proof. Re-query head/main and verify the envelope; any drift invalidates it and requires a new exact-head cycle.
...
**What**: Before any tag authorization, independently query and normalize the root-installed active `ProtectedReleaseTagV1` and its root-policy/event bindings. Require exact `refs/tags/release-set/**` namespace, creation/update/deletion/non-fast-forward rules, exactly the Ledger Writer Integration as sole always-bypass actor, stable pre/post rule digest, no current target tag/ref/Release collision, and a broker surface containing only canonical create-if-absent with no update/delete/force/general-name route. Run ordinary-user/token/Guardian/wrong-App create and Ledger-Writer update/delete dry negatives without creating a tag.
**Output**: Root policy/event and App identities; normalized tag ruleset/settings/digest before/after; broker code/policy/key fingerprint and route inventory; exact future tag name/object digest/absent-ref proof; negative principal/method receipts; and no-mutation proof.
**Acceptance**: Provider settings and broker policy together permit only one canonical absent-ref creation by Ledger Writer for the exact future tag and deny every other creator plus all update/delete/force routes. Any inactive/drifting rule, extra/missing bypass, collision, route overreach, or identity mismatch blocks Unit 66; no tag/Release/provider mutation occurs.

### ⬜ Unit 66: Protected Tag and GitHub Release Projection
**What**: Only after Units 65c-65d succeed for the current observed/effective `ReleaseSetPublished`, authorize and claim `release-tag-project-v1` bound to the stable tag-ruleset digest, deterministic `release-set/<ProductChangeID>/<publicationSHA>` name, exact canonical annotated-tag object bytes/OID, absent-ref prestate, publication/observation/main-pointer/attestation digests, and GitHub Release request. The Ledger Writer broker re-queries rules/ref/release, mints a bounded token, creates the annotated tag object and ref with create-if-absent only, then creates the GitHub Release and attaches sanitized attestations. Lost responses re-query exact equality; an exact existing object/ref/release is idempotent success, while any mismatch/collision contains. Revoke or contain token, re-query unchanged rules plus exact tag/ref/object/release, and append terminal. No update/delete/force API exists.
**Output**: Authorization/claim/terminal-or-containment commits; run/attempt/broker/token identities; normalized tag-ruleset digests before/after; canonical tag object/ref/name/OID and create receipt; collision/retry reconciliation; GitHub Release/attestation IDs and URLs; token revocation-or-expiry containment; update/delete route-absence proof; and authoritative post-query.
**Acceptance**: Guard rejects absent/aborted/stale generations, wrong namespace/object/principal, permissive/drifting rules, existing mismatch, and update/delete/force. Exactly Ledger Writer creates the absent canonical tag once under active `ProtectedReleaseTagV1`; retries prove exact equality; GitHub Release identifies the current Unit 64c publication and Unit 65c pointer; token is terminally handled; no private artifact and no alternate shipment authority.

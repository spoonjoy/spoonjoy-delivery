...
- [ ] `ReleaseSetPublished` is the direct child of `FinalizationClaim`, consumes a supervisor-signed local tombstone plus every nonce-bound source-runner cleanup attestation and bounded request-to-CAS provider evidence, contains the complete Release Set, and is the sole shipment event; projections are non-authoritative pointers.
...
**What**: Add red workflow/semantic tests for exact-SHA append workflow, expected-parent update, `ReceiverAcknowledged` actor/payload/idempotent retry/active-claim refusal, provider actor/run binding, claim/terminal/cancellation/finalization payloads, singleton environment governance, coordinator-owned claim plus independently registered abort-only recovery, attempt-scoped fsynced cleanup state machine/supervisor/signed tombstone, local and source-runner purge-before-publication, nonce-bound evidence ingest, operator/coordinator/machine disappearance, bounded monotonic request-to-CAS timing, ambiguous CAS recovery, Release Set proof graph, stale evidence, direct-child shipment, aborted finalization, and non-authoritative projections.
...
**What**: Implement `src/finalize.ts`, `src/private-cleanup.ts`, `src/projections.ts`, the cleanup supervisor/recovery CLI, `.github/workflows/ledger-append.yml`, `.github/workflows/finalization-coordinator.yml`, `.github/workflows/finalization-abort-recovery.yml`, `.github/workflows/finalization-evidence.yml`, `.github/workflows/project-release-set.yml`, nonce-bound attestations, and finalize/project commands. Before claim, coordinator registers exact-SHA independent recovery plus scheduled sweeper; recovery is abort-only and race-safe. The fsynced cleanup WAL and ephemeral supervisor signing key make tombstones independently verifiable. Publication requires verified local and source-runner purge receipts.
...
**What**: Cover every finalization rejection/race, original and recovery-run cancellation/loss, scheduled sweep, recovery/publication CAS ordering, failed query, cleanup WAL transition/crash/lock/supervisor/signature/restart/zero-write/purge failure, missing source-runner cleanup, bounded/noncached Date request and CAS deadlines, ambiguous ref result, projection retry, and attestation claim.
...
**What**: Without touching source code/provider state, wait for the release owner to push its tests-first handoff repair: canonical `outbound-owner-release.json` usage, strict `receiver-ack.schema.json`, `scripts/verify-release-ownership-handoff.rb`, consistent cleanup/evidence-index paths, and tests that bind receiver-ack content to the protected `ReceiverAcknowledged` ledger event. The payload is acyclic: it contains outbound commit/path/digest, ledger commit/payload digest, receiver IDs, and protected fields, but never its own containing delivery/upstream commit. Verifier arguments/query results supply those containing commits as external reachability envelopes. Inspect the exact remote commit and run gates read-only.
...
**What**: Add red tests for `production-deploy.yml` and `storybook.yml`: non-environment preflight, shared production-mutation concurrency, one protected mutation job with `environment: {name: production, deployment: false}`, required-review waiting despite no Deployment object, authorization/claim verification, no legacy push deploy, no `deployments: write`/`gitHubToken`/Wrangler Action/network install under credentials, exact prebuilt Wrangler `4.90.0` dependency tree, pinned Node `22.22.0`/`ubuntu-24.04`, least-privilege separate credentials, private sandboxed capture/cleanup receipt, and DAG containment hooks.
...
**What**: Split `.github/workflows/production-deploy.yml` into preflight plus claimed singleton operations. Make Storybook push/PR build-only. In an unprivileged pinned Node `22.22.0`/`ubuntu-24.04` preflight, install from the frozen lockfile, build Storybook, assemble and attest the exact Wrangler `4.90.0` dependency/tree plus static output, and record runner-image identity. The manual claimed deploy job uses `environment: {name: production, deployment: false}` and shared `web-production-mutation`, verifies the prebuilt bundle before injecting a Pages-only token into the single command, performs no install/action, uses controlled `TMPDIR`/`HOME` with private capture/sanitization/cleanup receipt, and emits no GitHub Deployment/status. Add governance branches and exact delivery validator.
...
**What**: Cover absent/stale/revoked claims, environment/custom-protection incompatibility, required-review waiting with `deployment: false`, Storybook build-only push/PR, Pages deploy/no-op/failure/containment, any GitHub Deployment/status side effect, wrong/tampered tool bundle/runner identity, post-token install/network attempt, sandbox/cleanup failure, shared-concurrency races, governance operations, every receipt/containment alternative, and log leaks.
...
**What**: Add red tests for source-owned exact-delivery-SHA dispatch, FinalizationClaim/nonce binding, least-privilege Worker/D1/R2/Pages/GitHub queries, exact Storybook project/deployment/source/build/tool-tree/Node/runner identity, absence of GitHub Deployment/status side effects, per-run sandbox manifest and attested cleanup receipt, no raw-file/credential/cache/artifact residue, cancellation/unverifiable-disposal failure, complete fingerprints/audit-watermarks, strict timestamp windows, sanitization, artifact attestation, expiry, and wrong run/ref/event.
...
**What**: Add the source-owned production attestor and final-query mode bound to claim/nonce. Each run creates its own cleanup context, sandboxes `TMPDIR`/`HOME`/downloads/caches, handles credentials and raw responses in memory where possible, append-before-write manifests any unavoidable private file, performs final inventory requery, and emits allowlisted evidence plus an attested terminal cleanup receipt. Query GitHub artifacts/caches after the run and reject unexpected objects; cancellation or unverifiable runner disposal aborts.
**Output**: Independent web/Cloudflare/D1/GitHub attestor with runner cleanup attestation.
**Acceptance**: Mock/live read-only dry runs and evidence/cleanup verification pass without mutation; missing/invalid cleanup or unexpected artifact/cache can never satisfy finalization.

### ⬜ Unit 16c: Web Provider Attestor - Coverage
**What**: Cover provider/API/attestation/redaction/retry/expiry plus runner sandbox, manifest, cleanup, artifact/cache inventory, cancellation, and unverifiable-disposal branches.
...
**What**: Add red tests for every current ASC upload/PATCH/POST/skip/409 alternative, typed receipt dataflow, drift, idempotency, partial failure, private stdout/stderr, no `tee`/raw replay, and per-run sandbox/manifests for ASC credential files, `TMPDIR`/`HOME`, downloads/caches, cleanup receipt, cancellation, and unexpected GitHub artifacts.
...
**What**: Refactor the publisher into authorized graph operations with private capture, sanitized receipts, typed runtime IDs, per-request revalidation, and containment. Each run uses a claim-bound sandbox, append-before-write manifest, controlled credential files/private roots, `always()` cleanup plus post-run artifact/cache inventory, and an attested cleanup receipt; cancellation or unverifiable disposal is containment, never success.
**Output**: Receipt-producing TestFlight publisher with runner cleanup attestation.
...
**What**: Cover all provider alternatives, failures, retries, redaction, credential/sandbox/manifest/cleanup, cancellation, artifact/cache inventory, and containment.
...
**What**: Add red tests for exact delivery SHA, FinalizationClaim/nonce binding for final-query mode, least-privilege ASC queries, app/build/group/metadata/notification state, complete fingerprints/audit-watermarks, strict `S/C/R` windows, sanitization, artifact attestation, and a per-run sandboxed cleanup context covering credentials/raw responses/`TMPDIR`/`HOME`/downloads/caches. Reject cancellation, unverifiable disposal, unexpected GitHub artifact/cache, missing cleanup receipt, expiry, and wrong run/ref.
...
**What**: Add source-owned read-only ASC attestor with final-query mode bound to claim/nonce. Each run creates an isolated cleanup context, keeps credentials and raw responses in memory where possible, append-before-write manifests unavoidable private files inside controlled `TMPDIR`/`HOME`, performs final inventory consistency requery, emits allowlisted observation proof plus an attested terminal cleanup receipt, and post-queries GitHub artifacts/caches. Cancellation or unverifiable runner disposal aborts rather than claiming zero residue.
**Output**: Independent ASC attestor with runner cleanup attestation.
**Acceptance**: Fixture/read-only dry run and evidence/cleanup verification pass without mutation; no raw tester/credential/cache residue or unclassified retained object.

### ⬜ Unit 23c: Native ASC Attestor - Coverage
**What**: Cover query/pagination/state/redaction/attestation/error plus runner sandbox, manifest, credential, cleanup, artifact/cache, cancellation, and unverifiable-disposal branches.
...
**What**: Re-query exact merged mains, production environment/shared mutation concurrency, required-review/no-custom-protection compatibility with `deployment: false`, least-privilege Worker/D1/Pages scopes, current Worker/Storybook Pages and negative GitHub Deployment/status state, exact prebuilt Wrangler dependency-tree/Node/runner identities, current/previous ASC identities, hardware, Codex host/model/tool digest, in-flight runs, and cleanup ownership.
...
**What**: Authorize, preflight, claim, required-review approve, and manually deploy exact merged Storybook output to Pages project `spoonjoy-storybook` through `environment: {name: production, deployment: false}` and shared concurrency. Verify the prebuilt Wrangler `4.90.0` dependency tree, Node `22.22.0`, runner-image identity, and static digest before token injection; run no action/install/network fetch afterward; privately capture/sanitize in a controlled sandbox; emit runner cleanup, provider post-query, and terminal receipt.
**Output**: Authorization/claim/terminal-or-containment commits, run/attempt/approval IDs, source/static/tool-tree/Node/runner receipt digests, Pages project/deployment/source IDs, negative GitHub Deployment/status query, runner cleanup attestation, authoritative post-query, and containment receipt if needed.
**Acceptance**: Exactly one claimed Pages mutation runs after Worker/canary terminals; required reviewer applies although no GitHub Deployment object exists; receipt binds every exact runtime/input/claim/run; no post-token install or raw output; token cannot mutate Worker/D1; Pages state and zero runner residue are green. Containment/failure is not proof success.
...
**Acceptance**: Exact source/Worker/Storybook Pages/build/tool-tree/Node/runner/pack/migration/capability state and runner cleanup match; no GitHub Deployment/status side effect; no attestor mutation; evidence fresh.
...
**What**: Attempt finalization with stale digest, wrong claim, mismatched provider identity, forged/missing local or runner cleanup, delayed/cached Date/CAS responses, and original-coordinator cancellation. Prove the independent recovery/sweeper aborts the exact claim and loses idempotently to any late original attempt.
**Output**: Rejected original/recovery run IDs, timing/cleanup error codes, ledger parent/terminal, race receipts, and no-shipment post-query.
**Acceptance**: No `ReleaseSetPublished`; every created test claim reaches `FinalizationAborted`; recovery never publishes; state changes only by explicit aborted test records.
...
**Acceptance**: Replay inherits all Unit 37a acceptance, binds exact source/build/tool-tree/Node/runner/claim/run and cleanup receipt, proves no GitHub Deployment/status side effect, reaches terminal success, and preserves shared mutation serialization; containment/failure blocks convergence.
...
**What**: In parallel, dispatch exact web/native read-only attestors and GitHub queries defined by every manifest `renewal_query` to refresh current source-main/check, environment/deployment-false policy, Worker/D1/R2/Storybook Pages/source/build/Wrangler-tree/Node/runner identities, negative GitHub Deployment/status state, attestor runner cleanup, runtime, ASC/build eligibility, cleanup-zero, and feedback state without rerunning mutations. Bind one GitHub-server observation time and provider-specific windows; feed the batch into a new Unit 60c29f pass.
...
**What**: Before any private query, initialize one enumerable `0700` cleanup root and an fsynced write-ahead attempt state machine `ALLOCATING -> OPEN -> SEALED -> PURGING -> PURGED`. Persist boot/process identity, random nonce, Ed25519 supervisor public key, manifest/root policy, and recovery locator before allocating private children; files are `0600`. Every writer holds a shared context lock and append-fsyncs the manifest before write; purge holds the exclusive lock. A supervisor survives caller death. Every process/restart enumerates both WAL entries and filesystem children, purging all prior-boot/orphan states regardless of lease. Before deleting the last private locator/key, atomically persist and fsync a sanitized supervisor-signed `PURGED` tombstone in the durable receipt root. Define signed zero-write/never-created receipts.
**Output**: Sanitized context/public-key/nonce/policy digests, fsynced WAL/lock/supervisor/restart proof, signed receipt fixtures, and private state excluded from repositories/artifacts.
**Acceptance**: WAL `ALLOCATING` and supervisor identity exist before private allocation; no writer bypasses shared lock/manifest fsync; caller death and restart converge every orphan to `PURGED`; tombstone verifies against the public key later bound into Unit 61. Crash injection after every transition/fsync/rename/delete leaves either discoverable private state or a valid durable tombstone, never neither. Partial construction routes to Unit 64b; no claim/provider mutation occurs.
...
**What**: Dispatch the exact protected coordinator after green Unit 60f4. Before claiming, it registers and verifies an independent exact-SHA abort-only recovery run in a separate concurrency/cancellation domain plus a scheduled active-claim sweeper, binding original run/attempt, expected parent, deadline, and recovery identities. Only then it CAS-appends `FinalizationClaim`, binding attempt-context digest, supervisor public key, repair/protected-record/freshness identities, and watches for 45 minutes. Original may publish/abort; recovery may only prove original terminal/cancelled or missing past deadline and CAS-abort idempotently. Any late loser re-queries head and exits without mutation.
**Output**: Coordinator and recovery run/attempt/workflow SHAs, scheduled-sweeper identity, absolute deadline, generation, parent/claim commit, context/public-key digest, canonical records identity, actor proof, and verified dual-waiting state.
**Acceptance**: Independent recovery is durably waiting in a non-cancelling concurrency domain before claim; failure to register creates no claim. Original loss/cancellation cannot strand it; recovery never publishes, requires provider-observed terminal/deadline proof, and loses safely to original/other recovery. If GitHub is unavailable, the first resumed scheduled sweep resolves overdue claims before any new claim/merge.
...
**What**: When Unit 63 is green, compile a single-use final-query plan bound to Unit 61's claim/coordinator/recovery/deadline, Unit 60f3a context/public key, source-runner cleanup requirements, nonce, exact identities, purge prerequisite, and timestamp/network contract. Strict-parse HTTP `Date` only as IMF-fixdate GMT at one-second precision. Per provider define first-page `S`, consistency-requery `C`, and immediately following GitHub `R`; require `S <= C`, `C-S <= 120000`, and `-5000 <= R-C <= 5000`. Reserve ten seconds for final CAS: require a nonce/cache-bypass GitHub time query and ref update with local monotonic request/response boundaries, strict three-second request deadlines, one-second maximum handoff, and ambiguous-result ref recovery. If Unit 63 is not green, emit a skip report.
**Output**: Validated plan/digest/nonce, coordinator/recovery/context/deadline bindings, source/local purge requirements, exact time budgets, and green or structured skip report.
...
**What**: On Unit 60f3a partial failure, any terminal failure from Unit 60f4/63b, and every Unit 64 exit, consume the exact WAL context. A green Unit 60f4/63b carries it forward. For purge, transition/fsync `SEALED -> PURGING`, acquire the exclusive lock, compare filesystem/registry/manifest, remove registered and unexpected files/downloads/caches, scan all controlled roots and open handles, atomically persist/fsync the supervisor-signed sanitized tombstone, then delete the final private locator/key and transition `PURGED`. Support signed zero-write/never-created paths. Separately verify every source final-attestor's nonce-bound cleanup receipt and post-run GitHub artifact/cache inventory.
**Output**: Supervisor-signed local tombstone plus web/native runner cleanup attestations or explicit uncertainty dispositions, manifest parity, WAL/crash-recovery proof, root/open-handle/artifact/cache scans, and terminal status `publication_clean` or `abort_recordable`.
**Acceptance**: Local tombstone signature matches Unit 61's public key and local cleanup is zero for either status. `publication_clean` additionally requires every source-runner cleanup/absence proof and is the only status Unit 64c/65 accepts. Missing/cancelled/unverifiable runner cleanup or unexpected GitHub residue produces only `abort_recordable`, signals `FinalizationAborted`, and may proceed only to Units 64a1-c with the uncertainty recorded honestly. Local purge failure blocks both statuses and retries.

### ⬜ Unit 64c: Authoritative Finalization Resolution
**What**: Only after green Unit 64 evidence and Unit 64b status `publication_clean`, ingest the sanitized package. Coordinator validates exact claim/context/public-key/evidence/local-tombstone/source-runner-cleanup identities and re-queries source/public state. Immediately before CAS, issue a unique-nonce noncached GitHub time query: record local monotonic `m0` before send and `m1` after full response, require `m1-m0 <= 3000`; strict-parse server Date `E`; for every provider `T` in `{S,C,R}`, `E-T < -5000` fails, negative values through `-5000` clamp to zero, and normalized age must be `<=110000`. Record `m2` before CAS with `m2-m1 <=1000`; require CAS response by `m3` with `m3-m2 <=3000` and total `m3-m0 <=7000`; strict-parse response Date `F`, bind it to the terminal proof, and query the exact ref after any ambiguous response. Append only direct-child publication when all preconditions are green; otherwise abort.
**Output**: Coordinator/recovery/evidence run IDs; context/evidence/local-and-runner-purge attestations; per-provider `S/C/R/E/F`, monotonic boundaries, cache-bypass/request IDs and decisions; parent; authoritative terminal commit; ambiguous-result ref query; actor/workflow proof; and post-query.
**Acceptance**: Monotonic budgets plus the 110-second pre-CAS ceiling prove every provider observation is at most 120 seconds old at actual CAS despite five-second skew/one-second Date precision. Missing/cached/late/unparsable Date, exceeded deadline, unresolved/mismatched ambiguous-result ref, invalid purge, or drift cannot publish. Original coordinator or abort-only recovery emits one direct-child terminal; only `ReleaseSetPublished` marks shipment/unlocks Unit 65. Publication is point-in-time proof, not a distributed lock.
...
**What**: Only when Unit 64c proves current `ReleaseSetPublished` already consumed Unit 64b's supervisor-signed local tombstone and all nonce-bound source-runner cleanup attestations, authorize and project that authoritative commit/digest plus sanitized cleanup index to protected main, then verify pointer and CI.
...
**Acceptance**: Guard rejects `FinalizationAborted`, missing, stale, or prior-generation Unit 64c state; receipt matches claim/purge; projection points to current `ReleaseSetPublished`; terminal or containment commit recorded; projection cannot alter shipment truth.
...
**What**: Only after Units 64c-66 prove current-generation publication/projections, read-only verify ledger/projections/providers/feedback/in-flight state; enumerate the cleanup WAL root, tombstone registry, local paths/open handles/caches, source attestor artifacts/caches, and runner cleanup attestations to reprove Unit 64b zero residue; update Desk/lessons/docs; retire clean task-owned branches/worktrees; notify Slugger; and report final inventory.

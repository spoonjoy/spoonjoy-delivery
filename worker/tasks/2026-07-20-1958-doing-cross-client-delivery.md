...
- [ ] A one-time audited `ledger-root-bootstrap-v1` ceremony installs distinct least-privilege Ledger Writer and Delivery Guardian Apps and permanently disables setup after Ledger Writer appends `RootAuthorityInstalled`. Ledger Writer is the sole `release-ledger` bypass actor; Guardian is zero-bypass and sole exact-provenance required-check actor. Both private keys/tokens remain outside Actions in purpose-specific OIDC brokers; same-Actions-App check spoofing, identity substitution, ordinary `GITHUB_TOKEN`, other Apps, direct, and self-declared writes/checks fail.
...
- [ ] Source-owned attestors use exact reviewed delivery SHA and purpose-specific brokers: ASC reads are GET-only with a dedicated read credential; mutations use a distinct app-scoped credential and exact-operation local broker. Provider keys/JWTs/generic leases never enter Actions or children; raw output stays private; sanitized attestations are dispatched/verified through authenticated `gh`.
...
**Prerequisite**: Re-query Unit 11c1's exact distinct inert Ledger Writer and Delivery Guardian App/installation/repository identities and fail if either has any usable key/token/client secret, webhook, OAuth callback, device flow, user authorization, non-metadata permission, bypass, source installation, or identity drift. Bind both sets of provider-returned IDs, inert manifest digests, target permissions/installations, and non-substitution invariant into the immutable authority policy, root intent, existing-App key targets, and every later App assertion before review/signing.
...
**What**: Open the delivery PR, require zero GitHub approving reviews, collect a fresh signed hostile-review envelope, run exact-head CI, and repair PR-only failures. After final head/review is frozen, create a protected GitHub artifact attestation `RootBootstrapEnvelopeV1` binding exact head, unsigned intent digest, `rootCodeManifest`/root-code/policy-projection/schema-projection digests, pinned SPKI fingerprint, detached-signature digest and verification result, expected current-main SHA, exact workflow provenance, CI/reviewer verdicts, actor, issuance/expiry, and no-mutation proof. Re-query head/main and verify the envelope; any drift invalidates it and requires a new exact-head cycle.
...
**Authority assertion**: `RootAuthorityInstalled` binds both exact App identities, installations, target permissions, broker code/policy/key fingerprints, Guardian check contract, zero-main-bypass/zero-human-approval rules, and Ledger-Writer-only ledger bypass; any missing/substituted authority fails before append.
...
**Source-owner prerequisite**: After Unit 12d publishes exact Guardian/Ledger authority, the retained source task alone installs/reapproves Guardian with read-actions/read-contents/write-checks on web/native, emits exact-provenance Guardian checks, changes both mains to zero approvals/zero bypass/Guardian-required checks, and proves post-state before producing its successor. This delivery task only verifies those changes.
**What**: Without touching source/provider state, treat repairs `2ad767db1a3237b12dcda45e0f398119e8d3e8f8`, `f324901fc246bf9743503ab5aa46f59f87ff230a`, and `7411307777e9828cea5dc1b33d62434ab4ea2fdd` as nonterminal; the last still publishes absolute paths, permits Integration-App main bypass, deadlocks the single operator with a positive human-review count, and trusts spoofable same-Actions-App contexts. Require a later explicitly reviewed remote successor after delivery root provides exact Guardian/Ledger authority IDs. `ProtectedMainV1` requires zero human approvals and exact Guardian-App checks whose broker re-queries workflow database ID/path, protected workflow blob SHA, event, head SHA, run/attempt/job, signed hostile-review envelope, and check payload. Normalize every classic/ruleset layer; require active PR/strict checks/no force-delete and empty user/team/role/admin/Integration bypass or App allowance sets. `ProtectedLedgerV1` derives only the Ledger Writer App ID from exact-byte protected delivery authority policy and `RootAuthorityInstalled`, requires exactly that App bypass, and permits no other writer. Guardian and Ledger identities cannot substitute. Public outbound/ack bytes use random stable worktree IDs/roles and reject private paths/unkeyed path digests. Negative tests cover positive human-approval counts, same-Actions-App wrong workflow/path/blob/event/head/run/attempt/job, forged/stale review envelope, wrong Guardian/Ledger App, coordinated acknowledgment/event/bypass substitution, any main bypass, and path leakage. Verify canonical schemas/checker, both mains, ledger, and exact ack bytes.
**Output**: Exact upstream successor/PR/reachability; paths/tests/schema/verifier digests; Guardian/Ledger authority IDs; repository-specific workflow/check/reviewer provenance; normalized stable `ProtectedMainV1`/`ProtectedLedgerV1` reports/digests; and coordination receipt.
**Acceptance**: Remote committed successor is independently green before Unit 13a1; both mains require zero human approvals, exact Guardian checks, and zero bypass while ledger has only exact Ledger Writer bypass. Same-Actions-App wrong-workflow evidence, wrong/missing/extra principal/source, permissive/drifting rules, branch-only evidence, path leakage, and self-reference fail; upstream still owns source; no source/provider mutation by this task.
...
**What**: Add red tests for every ASC operation/receipt/failure and a distinct OIDC-authorized exact-operation mutation broker. The Cloudflare broker holds the dedicated app-scoped mutation key at rest and releases it only in an HPKE capsule bound to the exact protected run/claim and pre-attested local broker key. The executable macOS broker on the actual `macos-26` path decrypts only into locked supervisor memory, accepts typed local RPC rather than generic HTTP/key access, constructs fixed Transporter upload argv and closed ASC POST/PATCH templates itself, captures every outgoing operation, and refuses caller URL/method/header/body/argv override. Its separate ephemeral child UID has no key/JWT/signer/unwrap/lease capability; peer ACL, task-port/ptrace/privilege denial, deny-by-default writes, supervisor lease, UID-wide process termination, detached-fork exhaustion, hostile alternate key/out-of-root/lock/signal/cancellation, rollback/containment, and no-secret verification all fail closed when unsupported.
...
**What**: Refactor publisher into authorized graph operations executed through the exact pre-attested macOS mutation broker. The Cloudflare authorization broker validates OIDC/claim/JTI/graph and encrypts the one-use mutation credential only to that broker's pre-bound key. The local broker decrypts into locked supervisor memory outside the child UID, verifies artifact/operation inputs, executes only fixed Transporter upload or allowlisted ASC POST/PATCH/skip templates, and emits signed receipts; it never exposes key/JWT/generic lease or accepts caller-composed requests. The supervisor also holds the claim-bound evidence signer and cleanup lease, applies deny-by-default filesystem sandboxing, blocks child signer/credential/task-port/ptrace/privilege access, terminates/re-enumerates the UID, destroys/fsync-verifies roots, and signs teardown. Separate no-secret job rejects alternate keys, verifies/uploads receipts, and inventories artifacts/caches. Typed runtime IDs, outgoing capture, revalidation, rollback/containment, cancellation, and unsupported-boundary failure remain exact.
...
**What**: Cover provider alternatives/failures/retries/redaction, exact Transporter argv and ASC HTTP request construction, mutation method/path/body/ID allowlists, caller override/SSRF/generic-lease denial, OIDC/JTI/capsule replay, plus live macOS broker binding, child credential/signer/unwrap/lease isolation, out-of-root writes, detached descendants, UID exhaustion, supervisor teardown, alternate-key rejection, separate verifier, cancellation, artifact/cache inventory, rollback, and containment.
...
**What**: Open the web PR with zero requested GitHub approvals, run exact immutable CI and a fresh hostile review, obtain Guardian-App checks bound to exact workflow/path/blob/event/head/run/attempt/job and signed review envelope, and repair PR-only failures without merge or deploy.
**Output**: PR/head/run/workflow IDs, review-envelope digest, Guardian check IDs/receipts, and terminal review state.
...
**What**: Open the native PR with zero requested GitHub approvals, run exact immutable CI and a fresh hostile review, obtain Guardian-App checks bound to exact workflow/path/blob/event/head/run/attempt/job and signed review envelope, and repair PR-only failures without merge or TestFlight.
**Output**: PR/head/run/workflow IDs, review-envelope digest, Guardian check IDs/receipts, and terminal review state.
**Acceptance**: Exact head green under Guardian CI/review checks; zero human approvals/bypass; no unresolved or in-flight check.
...
**What**: Through the now-proven `internal-testflight` environment's exact claimed credential-migration operation, authorize creation or exact reuse of two distinct ASC credentials limited to Spoonjoy app access: a read credential for the GET-only broker and a mutation credential for the exact-operation broker. Through isolated browser/provider and one-use Cloudflare secret-import operations, stream each credential into only its designated disabled broker binding, prove issuer/key/app-scope fingerprints, challenge the read broker with an allowlisted GET and the mutation broker with a signed no-provider-call dry run, then remove/requery every GitHub Actions/source/local copy and locally purge/unmount all transient material. Activate only after both inventories, non-substitution tests, and local-zero pass. Every partial failure rolls back new credentials/copies or disables and contains both brokers; append terminal-or-containment and post-query. If app-scoped credential creation is impossible with authenticated account tooling, record exact `BLOCKED_HUMAN` and do not activate either broker.
**Output**: Authorization/claim/environment-approval/terminal-or-containment commits; ASC issuer/key/app-scope fingerprints; Cloudflare import/challenge/inventory receipts; read/mutation non-substitution and mutation-no-call dry proof; source-secret/local-zero evidence; revocation/rollback/containment receipts; and authoritative post-query.
**Acceptance**: Exactly one claimed native job is approved under the environment gate; exactly one distinct app-scoped credential is active in each designated broker; neither exists in Actions/child/local storage; read broker can only GET; mutation broker can only issue exact one-use capsules to the attested local broker; dry proof performs no ASC mutation; every failure has terminal containment; no TestFlight upload/metadata/group/notification mutation occurs.

### ⬜ Unit 28: Merged-State Pilot Rebaseline
**What**: Re-query exact merged mains, zero-human-approval/Guardian-check main protection, production environment/shared mutation concurrency, required-environment-review/no-custom-protection compatibility with `deployment: false`, web/native distinct envelope-key generations/fingerprints/unwrap policy and zero bootstrap credentials, active distinct ASC read/mutation credential generations and broker/source-secret/local-zero state, least-privilege scopes, Worker/Storybook Pages and negative GitHub Deployment/status state, exact portable Node checksum/signature/OS/architecture plus Wrangler-tree/runner identities, current/previous ASC identities, hardware, Codex host/model/tool digest, in-flight runs, and cleanup ownership.
...
**Credential dependency**: Units 43-46 also directly depend on Unit 27e3's active mutation-broker credential/binding/policy/local-broker digests and Unit 47 directly depends on its active read-broker credential/binding/policy digest. Any rotation, disablement, source-secret recurrence, scope change, broker code/policy change, or inventory mismatch invalidates the dependent nodes and requires a new Unit 29 evaluation before claim.
...
**What**: Authorize, preflight, claim, and approve exact archive/export/upload. The Cloudflare mutation broker validates OIDC/claim/JTI and releases the one-use credential only encrypted to the pre-attested local mutation broker; that broker verifies archive/IPA hashes, constructs fixed Transporter argv, performs exactly one upload, destroys credential memory, and emits signed receipt. Verify provenance/build state through the independent read broker and append terminal.
**Output**: Authorization/claim/terminal-or-containment commits; run/attempt/broker IDs; captured Transporter argv digest; upload receipt; hashes/provenance; ASC app/build IDs/state; credential teardown; and read-broker post-query.
**Acceptance**: Receipt binds exact claim/graph/run/artifact/argv; no Actions job or child receives ASC key/JWT/generic lease; exact artifact uploads once and reaches required state or containment; terminal appended.

### ⬜ Unit 44: ASC Metadata and Localization Operation
**What**: Authorize export-compliance and app/build-localization predicate branches. The exact-operation mutation broker constructs only the selected allowlisted PATCH/POST/skip templates from validated read-broker outputs, executes under one-use custody, destroys credential memory, then the independent read broker post-queries before terminal.
**Output**: Authorization/claim/terminal-or-containment commits; run/attempt/broker IDs; captured PATCH/POST/skip request and receipt digests; credential teardown; and read-broker post-query.
**Acceptance**: Receipts bind exact claim/graph/run/request; one branch per node; exact resolved IDs; no generic credential/capability reaches Actions; terminal-or-containment appended.

### ⬜ Unit 45: ASC Group Attachment Operation
**What**: Authorize group create/attach/skip and execute only the chosen allowlisted request through the exact-operation mutation broker under one-use custody; destroy credential memory, post-query exact relation through the read broker, and append terminal.
**Output**: Authorization/claim/terminal-or-containment commits; run/attempt/broker IDs; captured group request/receipt digest/IDs; credential teardown; and read-broker post-query.
**Acceptance**: Receipt binds exact claim/graph/run/request; exact build attached once; no generic credential/capability reaches Actions; terminal-or-containment appended.

### ⬜ Unit 46: ASC Tester Notification Operation
**What**: Apply executable no-op by default, or authorize auto-notify/notification only if explicitly required and execute its exact allowlisted request through the mutation broker under one-use custody; destroy credential memory, post-query via read broker, and append terminal.
**Output**: Authorization/claim/terminal-or-containment commits; run/attempt/broker IDs; no-op or captured notification request/receipt digest; credential teardown; and read-broker post-query.
**Acceptance**: Receipt binds exact claim/graph/run/request; no implicit notification or generic credential exposure; chosen branch has terminal-or-containment append.
...
**What**: Re-query both ASC credential identities/scopes, source-secret absence, Cloudflare broker bindings/inventories, local-broker policy/key, and revocation state. Re-run Unit 27e3 under the protected native environment if any native/broker/policy/credential/source-secret input drifted; otherwise issue a validated no-replay receipt. Rotation creates a new generation, proves both brokers before cutover, retires old credentials, and terminally contains every partial failure; it performs no ASC product mutation.
**Output**: Dependency decision; fresh authorization/claim/environment-approval/terminal-or-containment, custody/inventory/non-substitution/local-zero/post-query evidence; or no-replay receipt.
**Acceptance**: No-replay requires exact active read/mutation broker custody and zero source/local copy; replay is protected, generation-monotonic, no-product-mutation, and terminal; any uncertainty invalidates Units 43-47 and blocks finalization.

...
- [ ] `ReleaseSetPublished` is the direct child of `FinalizationClaim`, consumes an attested zero-residue purge receipt plus CAS-time provider evidence, contains the complete Release Set, and is the sole shipment event; main/tag/GitHub Release projections point to it without becoming alternate authorities.
...
**What**: Implement `src/finalize.ts`, `src/projections.ts`, `.github/workflows/ledger-append.yml`, `.github/workflows/finalization-coordinator.yml`, `.github/workflows/finalization-evidence.yml`, `.github/workflows/project-release-set.yml`, nonce-bound GitHub artifact attestations, and CLI commands for authorize/revoke/claim/terminal/cancel/finalize/project. The protected coordinator appends the claim itself and immediately watches the entire claimed interval in the same run, accepts one exact sanitized evidence artifact only after a verified purge receipt, and appends `FinalizationAborted` on disappearance, timeout, invalid evidence, failed purge, or stale pre-CAS provider time.
...
**What**: Cover every finalization rejection, race, failed query, cleanup-context/manifest/trap/supervisor/expired-context/zero-write/purge failure, local death and machine-restart recovery, stale CAS-time query, projection retry, and attestation claim.
...
**What**: Add red tests for `production-deploy.yml` and `storybook.yml`: non-environment preflight, shared production-mutation concurrency, one protected mutation job/environment, authorization/claim verification, read-only environment governance operations, no legacy push-triggered Worker or Cloudflare Pages deploy, no `deployments: write`/`gitHubToken`/GitHub Deployment mutation, exact Wrangler `4.90.0`, least-privilege separate Worker/Pages credentials, DAG receipt/containment hooks, and private provider capture.
...
**What**: Split `.github/workflows/production-deploy.yml` into preflight plus claimed singleton operations. Refactor `.github/workflows/storybook.yml` so push/PR only build/test and can never use Cloudflare credentials; add manual claimed Pages preflight/deploy/receipt branches under the same `production` gate and shared `web-production-mutation` concurrency. Remove `deployments: write` and `gitHubToken`, pin `cloudflare/wrangler-action` to commit `ebbaa1584979971c8614a24965b4405ff95890e0` and `wranglerVersion: 4.90.0`, add read-only governance branches, pin exact delivery validator, use separate least-privilege Worker/D1 and Pages credentials, and privately capture provider output.
...
**What**: Cover absent/stale/revoked claims, environment mismatch, Storybook build-only push/PR, manual Pages deploy/no-op/failure/containment, forbidden GitHub Deployment/status side effects, missing/mutable/wrong Wrangler version, shared-concurrency races, governance authorization/verification with no provider access, every operation alternative/receipt/containment, and log leaks.
...
**What**: Add red tests for source-owned exact-delivery-SHA dispatch, FinalizationClaim/nonce binding, least-privilege Worker/D1/R2/Cloudflare Pages/GitHub queries, exact Storybook project/deployment/source/build/Wrangler `4.90.0` identity, absence of GitHub Deployment/status side effects, complete fingerprints/audit-watermarks, strict timestamp windows, pagination consistency, sanitization, artifact attestation, expiry, and wrong run/ref/event.
...
**What**: Re-query exact merged mains, production environment/shared mutation concurrency, least-privilege Worker/D1/Pages credential scopes, current Worker/Storybook Pages state, negative GitHub Deployment/status state, exact Wrangler action/runtime identities, current/previous ASC identities, hardware, Codex host/model/tool digest, in-flight runs, and cleanup ownership.
...
**What**: Authorize, preflight, claim, approve, and manually deploy exact merged Storybook static output to Cloudflare Pages project `spoonjoy-storybook` through the refactored workflow; enforce shared `web-production-mutation` concurrency, separate least-privilege Pages token, exact source/build/`wrangler-action` commit `ebbaa1584979971c8614a24965b4405ff95890e0`/Wrangler `4.90.0` identities, no GitHub Deployment/status mutation, private provider capture, post-query, and terminal append.
**Output**: Authorization/claim/terminal-or-containment commits, run/attempt ID, Storybook build/deploy/runtime receipt digests, Pages project/deployment/source IDs, negative GitHub Deployment/status query, authoritative post-query, and containment receipt if needed.
**Acceptance**: Exactly one claimed Pages mutation runs after Worker/canary terminals; receipt binds exact source/build/action commit/Wrangler/claim/run; deployed production branch points to exact SHA; token cannot mutate Worker/D1 and workflow cannot write GitHub Deployments; terminal success and post-query are green. Containment/failure is not proof success.
...
**Acceptance**: Exact source/Worker/Storybook Pages/build/action commit/Wrangler `4.90.0`/pack/migration/capability state matches; no GitHub Deployment/status side effect; no attestor mutation; evidence fresh.
...
**Acceptance**: Replay inherits all Unit 37a acceptance, binds exact source/build/action commit/Wrangler `4.90.0`/claim/run, proves no GitHub Deployment/status side effect, reaches terminal success, and preserves shared mutation serialization; containment/failure blocks convergence.
...
**What**: Before the first pre-claim/private query, create an attempt-scoped cleanup context independent of finalization generation: one random attempt nonce, one private temp directory outside every repository created atomically at mode `0700`, an exact append-before-write manifest with `0600` files, expected temp/cache roots, EXIT/INT/TERM cleanup trap, and a lease-bound local cleanup supervisor that survives caller death. Store its recoverable locator only in a `0600` private local registry; on machine restart, recovery scans that controlled registry and purges expired contexts before work resumes. Define schema-valid `zero-write` and `never-created` purge receipts.
**Output**: Sanitized attempt-context digest, nonce digest, manifest/root/lease policy digests, trap/supervisor/restart-recovery proof, zero-write/never-created receipt fixtures, and private registry state excluded from all repositories/artifacts.
**Acceptance**: Trap and supervisor are armed before any private write; caller death triggers purge, machine restart blocks resumed work until expired-context purge succeeds, and no raw nonce/path enters public output. A partial construction failure emits a bounded partial-context digest and routes directly to Unit 64b's `never-created`/zero-write proof before abort-record review. Every Unit 64b route carries an exact context digest; no finalization claim or provider mutation occurs.
...
**Output**: Attempt-context digest, attested pre-claim renewable batch, latest evaluation pass/time, durable/renewable predicate inventory, protected records identity, and green decision or loopback record.
**Acceptance**: Unit 61 starts only from one complete green latest pass bound to Unit 60f3 and Unit 60f3a; every renewable predicate is freshly queried; every private write is manifested; mismatch/unknown cannot be waived and reaches a green purge before records review.

### ⬜ Unit 61: Finalization Claim
**What**: Dispatch the exact protected `finalization-coordinator` after green Unit 60f4. In one uninterrupted workflow run, it expected-parent appends a fresh generation-reserving `FinalizationClaim`, binds the attempt-context/repair iteration/protected-record/freshness identities, and immediately enters its 45-minute fail-closed watchdog before exposing success. It alone may resolve the claim and expected-parent appends `FinalizationAborted` on caller disappearance, timeout, invalid evidence, failed purge, stale CAS-time evidence, or any non-publish terminal signal.
**Output**: Coordinator run/attempt/workflow SHA, absolute deadline, finalization generation, repair iteration, ledger parent/claim commit, attempt-context digest, canonical records identity, actor/workflow identity, and verified waiting-state query.
**Acceptance**: Claim creation and watchdog registration occur in one protected run with no fallible external step between them; generation is new; claim is head and blocks every other transition; coordinator is durably waiting before Unit 62; an aborted prior generation cannot be reused or stranded.
...
**What**: When Unit 63 is green, compile a single-use final-query plan bound to Unit 61's claim/coordinator/deadline, Unit 60f3a's attempt context, nonce, exact workflow/source/validator identities, evidence predicates, purge prerequisite, and timestamp contract. Strict-parse provider/GitHub HTTP `Date` only as IMF-fixdate GMT, convert to UTC epoch milliseconds at exact one-second precision, and serialize RFC 3339 with `.000Z`; missing/non-GMT/invalid/substituted artifact times fail. Per provider let `S` be first-page Date, `C` final full-inventory consistency-requery Date, and `R` the immediately following GitHub API Date. Require inclusive `S <= C`, `C-S <= 120000`, and `-5000 <= R-C <= 5000`. If Unit 63 is not green, emit a structured skip report.
**Output**: Validated final-query dispatch plan/digest/nonce, coordinator/context/deadline bindings, purge requirement, and green or structured skip report.
**Acceptance**: Plan is complete, single-use, source-owned-secret preserving, and executable by Unit 64 through authenticated local `gh`; it grants read-only attestation only and performs no query or mutation itself; it cannot outlive the coordinator deadline.

### ⬜ Unit 63b: Finalization Claim Resolution
**What**: If Units 62-63a are fully green, record a validated continue decision bound to the exact coordinator/generation/repair iteration/deadline and leave the coordinator waiting for Units 64-64c. Otherwise submit one attested failure signal; the coordinator expected-parent appends `FinalizationAborted`, verify it as head, execute Unit 64b with the exact attempt context, write only sanitized abort/purge evidence, then Units 64a1-c. Coordinator timeout independently aborts if the caller disappears.
**Output**: Continue-decision receipt, or coordinator terminal run/parent/`FinalizationAborted` commit/actor proof/post-query plus context-bound purge and loopback units.
...
**What**: After Unit 63b's green continue decision and while Unit 61's exact coordinator is waiting, use authenticated `gh` and the Unit 60f3a append-before-write context to dispatch final-query attestors, verify private Worker/D1/R2/Pages/ASC fingerprints/metadata/identities/attestations, recompile and leak-scan the Release Set, and stage one sanitized nonce-bound evidence package. Do not ingest it or append a ledger terminal yet. Any failure submits a terminal failure signal to the coordinator, verifies `FinalizationAborted`, and routes through Unit 64b before abort-record review.
**Output**: Coordinator/context identities; final attestor run/attempt/artifact/attestation IDs and provider `S/C/R` timestamps; sanitized evidence package/digest; refreshed Release Set/query/leak digests; or authoritative `FinalizationAborted` proof on failure.
**Acceptance**: Coordinator remains the claim owner; every private write was manifested; consistency hashes and Unit 63a query-window equations match; no raw/private artifact enters the sanitized package; on green the ledger still ends at `FinalizationClaim` and only Unit 64b may run next; failure/timeout cannot publish.

### ⬜ Unit 64b: Finalization Private-Evidence Purge
**What**: On Unit 60f3a partial failure, any terminal failure exit from Unit 60f4 or 63b, and every green/failure exit from Unit 64, consume the exact or bounded-partial attempt context, acquire its exclusive cleanup lock, compare the live private directory to the append-before-write manifest, remove every registered or unexpected attempt file plus downloads/caches and the directory, and scan expected roots for the attempt nonce. A green Unit 60f4/63b carries the still-open context forward instead of purging. If no private file was written or the directory was never created, emit the schema-valid `zero-write` or `never-created` receipt. Persist only a sanitized receipt with context/nonce digests, artifact classes/counts/digests, trap disposition, absence checks, and zero residue.
**Output**: Context-bound sanitized purge receipt/digest, manifest-count parity, explicit/zero-write/never-created disposition, trap result, expected-root absence scan, and zero-residue result.
**Acceptance**: Every registered/unexpected attempt artifact is absent; directory/manifest/private-registry entry are removed or proven never created; supervisor is retired; no open handle or cache residue; receipt passes evidence policy. On a green Unit 64 path, purge failure/timeout submits failure to the waiting coordinator and can produce only `FinalizationAborted`; publication and Unit 65 are impossible until one green purge receipt exists. On an abort path, Units 64a1-c remain blocked and purge retries until zero residue is proven; abort never waives cleanup.

### ⬜ Unit 64c: Authoritative Finalization Resolution
**What**: Only after green Unit 64 evidence and green Unit 64b purge, dispatch the protected nonce-bound `finalization-evidence` ingest with the sanitized Release Set and purge receipt. The waiting Unit 61 coordinator validates the exact claim/context/evidence/purge identities, re-queries source mains/checks/mutation runs and public Worker/runtime/Pages digests, then immediately strict-parses a fresh GitHub API Date as CAS time `E`. For each provider and each `T` in `{S,C,R}`, let `A_T=E-T`: `A_T < -5000` fails; `-5000 <= A_T < 0` clamps to zero; `0 <= A_T <= 120000` passes; `A_T > 120000` fails. It expected-parent appends exactly one direct-child `ReleaseSetPublished` only when every check is green, otherwise `FinalizationAborted`.
**Output**: Coordinator/evidence run IDs; context/evidence/purge attestation IDs; per-provider `S/C/R/E` values and boundary decisions; ledger parent; authoritative `ReleaseSetPublished` or `FinalizationAborted` commit; actor/workflow proof; and post-query.
**Acceptance**: `E` is fresh, strict IMF-fixdate GMT, and obtained immediately before CAS; all inclusive skew/age boundaries and consistency hashes pass; purge receipt proves zero private residue before publication. Coordinator emits exactly one direct-child terminal even if local disappears. Only `ReleaseSetPublished` marks shipment and unlocks Unit 65; abort writes sanitized records through Units 64a1-c. Publication is age-bounded point-in-time proof, not a distributed lock.

### ⬜ Unit 64a1: Finalization-Abort Records Review
**What**: Conditional on Unit 60f3a, 60f4, 63b, 64, 64b, or 64c failure/abort and only after green Unit 64b, integrate exact current delivery main into `finalize-iN` with `git merge --no-ff origin/main`, add sanitized pre-claim/finalization/coordinator/abort/purge evidence and a byte-complete forward-port manifest, and hostile-review public-safe records. On successful Unit 64c publication this unit is skipped.
...
**What**: Only when Unit 64c proves current `ReleaseSetPublished` already consumed Unit 64b's zero-residue receipt, authorize and project that authoritative commit/digest plus sanitized purge receipt to protected main, then verify pointer and CI.

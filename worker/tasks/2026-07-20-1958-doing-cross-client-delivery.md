...
**What**: Without touching source/provider state, treat repairs `2ad767db1a3237b12dcda45e0f398119e8d3e8f8` and `f324901fc246bf9743503ab5aa46f59f87ff230a` as nonterminal. Independently verify remote-reachable candidate successor `7411307777e9828cea5dc1b33d62434ab4ea2fdd` or a later explicitly reviewed upstream replacement before acceptance. `ProtectedMainV1` consumes repository-specific allowlists of exact `{context, expected App ID}`, normalizes every classic/ruleset layer, requires active PR/strict checks/no force-delete/no broad bypass, and verifies stable digests; wrong positive App IDs/spoof checks fail. `ProtectedLedgerV1` derives the dedicated ledger App ID only from exact-byte protected delivery authority policy and `RootAuthorityInstalled` artifacts whose ref/commit/path/SHA-256 and mutual policy/App binding are re-queried, treats source/acknowledgment/event App IDs as redundant equal echoes, requires active mutability blocking and exactly that anchored App bypass, and permits no user/team/role/admin/other-App/direct writer path. Negative tests substitute acknowledgment, ledger event, and sole live bypass together and must still fail on the protected source anchor. Verify canonical handoff schemas/checker, acyclic ledger binding, both mains, ledger, and exact ack bytes.

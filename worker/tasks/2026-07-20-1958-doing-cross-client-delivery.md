...
**What**: Add red tests for CI, pinned Actions, attestations, `ProtectedMainV1`/`ProtectedLedgerV1`, exact root-intent signature/digest domains, concrete journal transport/genesis/resume/rollback/kill switch, sole App bypass, and OIDC broker identity/actor/transition enforcement. Model `token_mint_started/token_issued/token_revoke_started`, conservative expiry containment, bounded-byte mailbox, prepare/commit/status-observe, and every crash. Live negatives include wrong positive check App IDs, extra ledger bypass principals, legal OIDC with illegal event, unauthorized rerun, caller-created Git objects, and substituted/oversized package bytes.
...
**What**: Add SHA-pinned CI/resolver. Define `ProtectedMainV1` with repository-specific `{context, expected App ID}` checks, and `ProtectedLedgerV1` with exactly the ledger App bypass and no ordinary writer path; both normalize/stability-check all layers. Implement serialized broker identity/actor/transition validation. Before token POST, persist `token_mint_started` with no-cache GitHub Date, a 15000ms mint deadline, and conservative `containUntil = Date + 3675000ms` (provider one-hour lifetime plus request deadline and 60s skew); lost response globally blocks broker through that time. On response persist provider `expires_at` plus HMAC fingerprint as `token_issued` before Git work. Token is one-repository `contents:write`/`actions:read`, memory-only; persist `token_revoke_started`, revoke in `finally`, require `204`, else retain/retry while live or globally contain until expiry. Operation states are `prepare -> token_mint_started -> token_issued -> prepared -> committing -> cas_succeeded|cas_uncertain -> observed|abort_required -> token_revoke_started -> token_revoked|token_contained`, with OIDC bounded-byte mailbox/status recovery.
...
**What**: Cover resolver/transition bypass, actors/reruns/jobs/approval/OIDC, token scope and crashes before/after mint dispatch/response persistence/Git work/revoke dispatch/response, conservative global containment and expiry release, mailbox missing/substituted/oversized/caller-blob cases, CAS/ref-observation races, ruleset mode/wrong-positive-source/extra-bypass/digest drift, APIs, permissions, and broker drift.
**Output**: Coverage and warning logs.
**Acceptance**: 100% coverage, all delivery gates green, zero warnings.

### ⬜ Unit 11d: Root Bootstrap Intent and Dry Run
**What**: Read-only capture prestate and compile acyclic `RootBootstrapIntentV1` as RFC 8785 canonical JSON. Its `rootCodeDigest` hashes a UTF-8-byte-sorted `(path,SHA-256)` manifest of `src/`, `schemas/`, `policy/`, `scripts/`, `.github/workflows/`, package/lock/TS/Wrangler configs, excluding `.git`, `worker/`, build/coverage/temp and the intent/envelope/journal themselves; bind each policy/schema digest separately. Sign domain `spoonjoy-root-bootstrap-intent-v1\0 || canonical-bytes` with pre-enrolled Secure Enclave P-256 key `com.spoonjoy.delivery.root-bootstrap-v1`; pin SHA-256 DER public-key fingerprint in `policy/root-bootstrap-key.json`. Bind prestate/targets, `ProtectedMainV1`/`ProtectedLedgerV1`, exact genesis/root event, rollback, and states including `REMOTE_JOURNAL_CREATED`. Predeclare zero-parent journal genesis on `refs/heads/root-bootstrap-journal`, created/advanced by actor `16390116` through expected-parent Git Data API CAS. Journal CAS calls alone are exempt from self-journaling because each commit itself carries the next mutation intent; every external nonjournal call requires local append/fsync, remote intent CAS, call, remote receipt CAS, and requery CAS. Test every boundary.
**Output**: Canonical signed acyclic intent, path manifest/key fingerprint/signature proof, exact prestates/targets/genesis/events, journal ref/genesis/commit schemas, full crash/reconcile table, and zero-mutation proof.
...
**What**: Without touching source/provider state, treat repair `2ad767db1a3237b12dcda45e0f398119e8d3e8f8` as nonterminal and require its upstream-owned successor implementing the same delivery contracts. `ProtectedMainV1` consumes repository-specific allowlists of exact `{context, expected App ID}`, normalizes every classic/ruleset layer, requires active PR/strict checks/no force-delete/no broad bypass, and verifies stable digests; wrong positive App IDs/spoof checks fail. `ProtectedLedgerV1` requires active mutability blocking, exactly the dedicated ledger App bypass, and no user/team/role/admin/other-App/direct writer path. Verify canonical handoff schemas/checker, acyclic ledger binding, both mains, ledger, and exact ack bytes.
...
**What**: Compile one plan bound to claim/operation/coordinator/recovery/deadline, frozen jobs/bootstrap, cleanup, nonce, and mailbox. Per provider define `S` as no-cache GitHub Date immediately before query, `C` as no-cache GitHub Date immediately after response, and `R` as provider response Date; strict IMF-fixdate, require `S<=C`, `C-S<=120000ms`, `-5000<=R-C<=5000ms`. Mailbox workflow input carries base64 of RFC 8785 canonical, fully sanitized Release Set bytes (maximum 32768 bytes before base64), digest, operation ID, and no blob/tree/commit locator; broker decodes/strict-parses/rescans and alone creates Git objects. For each invocation persist unique clock-epoch ID and `performance.now()` offsets, never compare offsets across epochs. Broker `E` uses exact nonce/ref endpoint plus no-cache headers/status rules; for each `T in {S,C,R}`, fail `E-T < -5000ms`, clamp permitted negative skew to zero, require normalized `E-T<=120000ms`; enforce 10s E and 5s response-to-CAS dispatch, persist `committing`, 15s CAS. `F` is response Date/timing only. After any dispatch, state is `cas_uncertain` until authoritative ref: candidate => observable even after deadline; exact unchanged claim => expected-parent abort may race safely; legal other head => classify. Each `H` invocation is <=10s and starts within 5s when same epoch; across restart use new epoch and HTTP Date. Retry bounded invocations without total count while unavailable. Revoke/contain token; forbid atomic/CAS-age claims.
...
**What**: After Unit 63b green, approve/continue only exact Unit 60f4 jobs. Each revalidates identities before secret access. Verify claim-bound teardown and Worker/D1/R2/Pages/ASC fingerprints, formal `S/C/R`, manifests/object inventories; recompile/leak-scan. Produce RFC 8785 canonical fully sanitized package bytes <=32768 containing Release Set and future promoted-tombstone slot/schema; stage only these public-safe bytes/digest for Unit 64b completion and later mailbox input. Reject any Git blob/tree/commit locator or caller-created object. Cancellation/rerun/drift/oversize marks `abort_required`, then Unit 64b.
**Output**: Coordinator/recovery/context; exact attestor/evidence/cleanup IDs; provider `S/C/R`; bounded canonical sanitized bytes/digest; Release Set/query/leak digests; or abort proof.
...
**Output**: Final signed tombstone plus lock-gated `PROMOTED` WAL proof, canonical public package with tombstone filled and revalidated under 32768 bytes, isolation/descendant exhaustion, source teardown or uncertainty, manifest/crash/root/handle/object scans, and `publication_clean` or `abort_recordable`.
...
**What**: After `publication_clean`, dispatch exact protected-main OIDC mailbox relay carrying only bounded canonical sanitized bytes/digest/operation ID, then approve its `ledger-writer` job. Reject any caller blob/tree/commit. Broker rescans bytes, verifies package/bootstrap/claim/PROMOTED and formal time equations, persists pre-mint containment, creates blob/tree/candidate itself, executes E/CAS, records `F`, and enters `cas_uncertain` after dispatch until authoritative ref observation. Status/observe performs as many separately bounded `H` invocations as needed across restarts. Candidate is observed even after deadline; exact unchanged claim permits expected-parent abort CAS, whose race with a late candidate CAS has one winner; other head follows legal transition. Protected relay/recovery turns broker observation into `PublicationObserved`. Token revokes with `204` or global expiry containment.

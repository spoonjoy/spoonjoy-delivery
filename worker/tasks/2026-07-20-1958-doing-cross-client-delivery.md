...
**What**: Add source-agnostic red tests for static templates, typed topological dataflow, authoritative-query/prior-receipt inputs, branch cardinality, template/resolved digests, per-request drift, idempotency, retry, compensation, partial failure, typed rollback-target eligibility/priority/setup/refusal, the reserved bootstrap interactive node sequence, same-identity/prestate/target-only bootstrap resume, the installed-gate governance successor, and restore-only drift recovery. Use fictional provider fixtures only; exact web/native provider-operation inventories wait for the validated Unit 13 rebaseline.
...
**What**: Implement `src/operation-graph.ts`, `src/receipts.ts`, deterministic rollback-target selection/setup planning, fixed source-agnostic governance graphs, canonical expression resolution, dry-run plans, apply-time revalidation, receipt chaining, and containment planning. Rollback selection records every candidate, eligibility reason, freshness, priority, selected target, required claimed setup, and forbidden mutation; no eligible target fails closed. Interactive governance nodes are limited to bounded GitHub environment evidence/changes; provider/source node kinds are structurally forbidden from those graphs.
...
**What**: Run the typed selector at a GitHub-server timestamp no more than 120 seconds before apply. Priority: (1) reuse Unit 31's exact staged 0%-traffic deployment when its candidate/predecessor/graph still match; (2) otherwise authorize/claim/create a new 0%-traffic deployment from the exact candidate/predecessor and terminally record setup; (3) otherwise fail closed. Production traffic changes are forbidden. Authorize and live rollback/restore the selected target; simulation is negative-only.
...
**What**: Run the typed selector at a GitHub-server timestamp no more than 120 seconds before apply. Priority: (1) reuse an exact run-scoped subject/version override from Units 37/40 when graph/subject ownership still match; (2) otherwise authorize/claim/create a run-owned test subject and isolated override with exact cleanup; (3) otherwise fail closed. Global capability mutation is forbidden. Live disable/restore the selected target with oracle/terminals; simulation is negative-only.
...
**What**: Run the typed selector at a GitHub-server timestamp no more than 120 seconds before apply. Priority: (1) use an existing run-owned non-selected eligible sacrificial build with exact source/pack; (2) otherwise authorize/claim/archive/upload a new signed sacrificial build from exact native source/pack and a fresh build number, terminally recording setup; (3) otherwise fail closed. Selected-candidate mutation/deletion is forbidden. Live-supersede the chosen sacrificial build; simulation is negative-only.
...
**What**: Run `replay evaluate` against the exact protected proof-dependency manifest, baselines, current repaired identities/input digests, and predecessor outcomes. Set `decision_at` from a fresh GitHub API response `Date`. Mark `replay_required` when any input changed/is missing/is unknown/is expired at `decision_at`, any finite `max_age_seconds`/`expires_at` is absent, or any predecessor is required/failed; emit `no_replay` only with byte-equal maps, unexpired temporal fields, and green predecessors.
**Output**: Attested `replay-evaluation-iN.json` with every node 30-59, `decision_at`, old/current direct-input digests and temporal fields, predecessor closure, decision/reason codes, earliest decision expiry, original acceptance reference, and manifest/tool/iteration digests.
...
**What**: After all required replays, run `replay evaluate --freshness-only` with a new GitHub API `Date`, refreshed replay outputs, and every prior no-replay receipt. If any node is now expired/unknown/failed, propagate required status and return to the earliest affected Unit 60c30-59; repeat until one evaluation marks every node green simultaneously.
**Output**: Final attested current-iteration replay evaluation, authoritative `decision_at`, earliest remaining expiry, loop/replay history, and complete green node set.
**Acceptance**: All nodes 30-59 are green at one authoritative instant; no expired no-replay survives; any failure/containment loops rather than reaching Unit 60d.
...
**What**: Only on Unit 60d non-convergence, rebase/merge exact current delivery main into current `repair-iN-rK` without dropping records; add findings, proof-node failures, amendment/replay scope, and exact forward-port manifest; validate and hostile-review the complete failed-iteration records without provider mutation.
...
**What**: Immediately before Unit 61, re-run `replay evaluate --freshness-only` from protected Unit 60f3 records using a fresh GitHub API `Date`; verify provider/post-query/cleanup evidence is within 900 seconds, actor/hardware/runtime proof within 3600 seconds, and all stricter node limits. If any node expired or changed, increment repair iteration and loop to Unit 60a for mechanical replay and a new protected records checkpoint.
**Output**: Attested pre-claim evaluation/time, node ages/expiry, protected records identity, and green decision or loopback record.
**Acceptance**: Unit 61 may start only from one complete green evaluation bound to Unit 60f3; stale/unknown evidence cannot be waived or carried into a claim.
...
**Acceptance**: Coordinator is durable before local work and emits exactly one direct-child terminal even if local disappears. Immediately before CAS it strict-parses fresh GitHub API Date as `E` using Unit 63a rules. For every provider independently require inclusive `E >= C-5000` and `E >= R-5000`; let raw age `A=E-S`: `A < -5000` fails, `-5000 <= A < 0` clamps to `0`, and `0 <= A <= 120000` passes, with exactly `-5000`, `0`, and `120000` accepted and any larger magnitude rejected. Artifact creation/upload time never determines freshness; paginated consistency hashes must match. Source/public drift and timestamp/query uncertainty abort. `ReleaseSetPublished` alone unlocks Unit 65; abort increments repair iteration and loops to Unit 60a with fresh generation. Publication binds point-in-time observations, not a distributed lock; later external events are new changes.

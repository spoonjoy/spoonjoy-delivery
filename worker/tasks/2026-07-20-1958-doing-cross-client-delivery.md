...
**What**: Implement `src/operation-graph.ts`, `src/receipts.ts`, deterministic rollback-target selection/setup planning, fixed source-agnostic governance, repository-distinct supervisor-envelope start/provisional-challenge/local-zero/provision-or-abort/rotate/retire graphs, distinct inert Ledger Writer and Delivery Guardian registration/permission-elevation/key-import graphs, reviewer-key provision/rotate/revoke plus one-use envelope-consumption graphs, Guardian exact-provenance check emission, a post-root delivery-governance/broker-upgrade graph, root-authorized `source-guardian-cutover-v1`, canonical expression resolution, dry-run/revalidation/receipt/containment, and `ledger-root-bootstrap-v1`. Key graphs bind Cloudflare secret prestate, provisional/active event generation, canonical X25519 key/fingerprint, exact Started commit, byte-identical repository/generation/key/fingerprint plus challenge/local-zero receipts in Provisioned, exact challenge/ordinary OIDC allowlists and non-consuming wrong-kind branches, active-run retention, bootstrap credential, delete/contain alternatives, and terminal. Identity registration permits only metadata-only/no-webhook/no-key/no-OAuth/no-bypass creation, records distinct provider IDs, and on ambiguity proves exact inert identities or removes them. Elevation permits only intent-bound Ledger Writer delivery `contents:write` plus `actions:read`/`checks:read`, and Guardian `actions:read`/`contents:read`/`checks:write`, with zero main bypass, exact installation reapproval, isolated RAM-volume key import, rollback, and local-zero. Guardian check emission accepts only exact workflow provenance and a canonical valid unconsumed reviewer envelope; it cannot append ledger. Source cutover binds exact root event, source repository/App/installation IDs, prestates and target permission/check/rule digests; journaled retries may only resume the same target, exact rollback restores prestates before terminal, ambiguous state contains both source mains, and only exact post-query appends `SourceGuardianCutoverCompleted`. Post-root upgrade binds exact root event and live settings/broker prestate, permits reviewed deltas/rollback/disable containment, and forbids root identities/genesis changes. The root graph binds both exact pre-registered identities, reviewer key, source-cutover target contract, and repository/broker/ruleset/environment prestate; emits one Ledger-Writer-authenticated `RootAuthorityInstalled`; verifies setup removal; and thereafter rejects root replay. Candidate selection retains documented total-order/fail-closed rules.
...
**Acceptance**: The exact reviewed tree proves two distinct metadata-only/no-webhook/no-key/no-OAuth/no-device-flow/no-user-authorization/no-bypass App targets, repository-install separation, total ambiguous-response cleanup, a workflow-inaccessible non-exportable reviewer signer, and a distinct workflow-inaccessible governance signer whose observer cannot mutate and cannot sign incomplete/unstable ruleset evidence. Reviewer envelopes and ruleset snapshots cannot be forged, replayed, retargeted, cross-signed, or used after expiry/revocation. All gates are green at 100% coverage with zero warnings; no App/provider mutation has occurred; V1 records reviewer integrity/freshness without claiming separation from the sole operator.
...
**Prerequisite**: Re-query Unit 11c1's exact distinct inert Ledger Writer and Delivery Guardian App/installation/repository identities and Unit 11c0's exact reviewer/governance-observer SPKI/fingerprint/generations; fail if either App has any usable key/token/client secret, webhook, OAuth callback, device flow, user authorization, non-metadata permission, bypass, source installation, or identity drift, or if either local key is exportable/unavailable/substituted. Bind both provider identity sets, both local signers, inert manifest digests, target permissions/installations, source-cutover targets, and non-substitution invariant into the immutable authority policy, root intent, existing-App key targets, and every later assertion before review/signing.
...
**Authority binding**: The reviewed head, immutable policy, root intent, and `RootBootstrapEnvelopeV1` must all carry the exact provider-returned Unit 11c1 Ledger Writer and Delivery Guardian App/installation/repository IDs, inert/target manifests, repository installations, permission-reapproval plan, and isolated key ceremonies; any identity/capability drift invalidates the envelope and returns to review.
...
**Prerequisite**: Re-query both exact keyless/no-bypass Unit 11c1 identities and Unit 12b binding. This unit may only elevate/reapprove and key those existing reviewed Apps; it never creates, substitutes, or installs Ledger Writer on another repository. Any identity, permission, webhook, key-inventory, source-installation, or bypass drift blocks and returns to Unit 12a.
**Release-tag live governance**: In the same reviewed root journal, install/reconcile active `ProtectedReleaseTagV1` for recursive `refs/tags/release-set/**/*` with creation/update/deletion/non-fast-forward rules and exactly the Ledger Writer Integration as sole always-bypass principal. Prove the exact canonical two-level example matches under GitHub `FNM_PATHNAME`. Bind/challenge the distinct Unit 11c0 governance-observer SPKI/generation without exporting its private key. In one authenticated operator session, execute only no-cache `GET /user`, exact delivery collaborator-permission GET, and policy-pinned exact ruleset GET; require three fresh `200`s, exact `16390116`/`arimendelow` user and collaborator identities, built-in `permission: admin` and `role_name: admin`, bound canonical bodies/Date/request IDs, complete bypass actors, bounded skew, and stable ruleset ETag; then sign/verify one `ProtectedReleaseTagSnapshotV1`. Verify normalized digest/ETag stability around an authorized broker dry proof; switched account, custom role, `304`, cache hit, stale response, ordinary user/token/Guardian/wrong App creation, observer mutation/general read, implicit-tag Release POST, and every update/delete/upload route fail. No tag or Release is created.
**What**: Only after Unit 12a/12b convergence, verify and consume the exact acyclic intent plus `RootBootstrapEnvelopeV1`. Execute/reconcile the signed genesis transaction and journal every nonjournal call. CAS-install the envelope-bound head on expected unprotected main. Update/reapprove Ledger Writer to delivery-only `contents:write` plus `actions:read`/`checks:read`, and Guardian to delivery `actions:read`/`contents:read`/`checks:write`, with no other permission or bypass; broker-minted installation tokens must carry exactly those repository-scoped permissions, with Contents as Ledger Writer's sole write authority and no Administration permission. For each App, configure authenticated headed-browser download behavior to a distinct isolated encrypted RAM-backed APFS volume, generate exactly one private-key credential through GitHub's supported UI, stream the downloaded PEM into its disabled broker binding, query inventory/fingerprint, challenge exact App/installation/repository/permission identity, then purge/unmount/destroy the volume and prove local-zero. No PEM path/content enters arguments/logs/artifacts. Create/revoke one-use Cloudflare bootstrap credentials around import; lost download/import/reapproval/revocation responses reconcile from provider inventory or roll back to keyless metadata-only state. Import the exact reviewer SPKI/generation/revocation head into Guardian policy and exact governance-observer SPKI/generation/revocation head into tag policy without exporting either private key; challenge each disjoint canonical domain and one-use consume it. Apply zero-human-approval/zero-bypass `ProtectedMainV1` requiring exact Guardian CI/review checks, apply Ledger-Writer-only ledger rules/environment, and have the verified Ledger Writer broker create exact zero-parent ledger genesis. Guardian broker can emit checks but cannot append ledger; Ledger Writer cannot satisfy Guardian contexts. Stop at `GENESIS_CREATED`.
**Output**: Full local/remote journal chain; signed genesis/object/ref reconciliation; exact-main/CI; distinct App/install/permission/key fingerprint inventories; installation reapproval receipts; RAM-volume/download/import/purge/unmount/local-zero proofs; scoped Cloudflare bootstrap credential revocation or containment; distinct broker challenges; reviewer/governance-observer signer inventories and domain-separated challenges; Guardian rules/check dry proof; ledger rules/environment/genesis identities; normalized `ProtectedReleaseTagV1` settings/digest/ETag, canonical three-response identity/permission/ruleset snapshot, and create-only/empty-assets/implicit-tag negatives; per-call reconciliation; or exact rollback proof.
**Acceptance**: Live application occurs only after reviewed exact head. Main requires zero human approvals, zero bypass, and exact Guardian checks; journal/ledger genesis match intent and only Ledger Writer bypasses ledger. The release-tag namespace is actively protected with exactly Ledger Writer as bypass, but its broker can only create an absent canonical tag and then a body-only empty-assets Release after the exact ref exists; it cannot update/delete/force/upload or cause implicit tag creation. The distinct three-GET-only observer proves exact authenticated user, built-in repository-admin permission, and complete stable ruleset state from one fresh provider bundle and cannot mutate or general-read; no tag or Release exists yet. Exactly one challenged key per App exists only in its broker binding; identities/permissions/local signers cannot substitute; no PEM, mounted volume, local/bootstrap credential, or uncertainty remains. Broker endpoints remain root-only; no `RootAuthorityInstalled` yet; every injected crash converges to exact target, inert rollback, or containment.

### ⬜ Unit 12d: Protected Ledger Append Live Proof
**Authority assertion**: `RootAuthorityInstalled` binds both exact App identities, installations, target permissions, broker code/policy/key fingerprints, reviewer-attestation and governance-observer SPKI/generation/revocation/domain contracts, Guardian check contract, root-authorized `source-guardian-cutover-v1` graph and target digests, zero-main-bypass/zero-human-approval rules, Ledger-Writer-only ledger bypass, and stable `ProtectedReleaseTagV1` plus its Ledger-Writer-only canonical create-if-absent/body-only-empty-assets contract and one-use three-GET identity/permission/ruleset snapshot contract; any missing/substituted authority fails before append.
**What**: Continue the journaled ceremony from exact `GENESIS_CREATED`. Broker appends exact `RootAuthorityInstalled` as genesis child and durably records/revokes its token. That event is the permanent bootstrap kill switch: every bootstrap binary/version first observes ledger and refuses install/resume/rollback mutation after it, even if broker cleanup/redeploy was interrupted; only bounded post-root credential/path deletion and verification may continue through bootstrap code. The separately versioned ordinary-governance broker may execute only root-authorized post-root DAGs, including exact `source-guardian-cutover-v1`; it cannot call or revive any bootstrap node. Prove token revocation `204` or block all operations until the real one-hour expiry plus containment. Remove setup paths, prove reentry fails, then execute/contain one disposable non-shipping append while direct, `GITHUB_TOKEN`, other App, wrong workflow/environment/actor/rerunner, illegal event, and exfiltration attempts fail. Rotate clean worktree to `worker/cross-client-delivery-records-r0` only after `VERIFIED`.
...
**Credential dependency**: Units 43-46 also directly depend on Unit 27e3's selected custody mode, active mutation-broker credential/binding/policy/local-broker/Transporter-compatibility digests, and per-run mutation-recipient contract; Unit 47 directly depends on its active read-broker credential/binding/policy digest. Any rotation, disablement, source-secret recurrence, role/app-access/team-scope change, broker code/policy change, Transporter identity/compatibility change, mutation-recipient contract change, or inventory mismatch invalidates the dependent nodes and requires a new Unit 29 evaluation before claim.
...
**What**: Apply executable no-op by default. If auto-notify/notification is explicitly required, create/purge and claim-bind the Unit 22 per-run mutation recipient in credential-free preflight, recover it only by direct supervisor-to-local-broker transfer after approval, and execute the exact Spoonjoy-bound allowlisted request from a credential capsule encrypted to that recipient. Destroy recipient/credential memory, post-query via read broker, and append terminal.
...
**What**: Permanently forbid Unit 11d/root-bootstrap replay after `RootAuthorityInstalled`. Re-query exact delivery main, authority/root event, normalized repository/main/ledger/tag-ruleset/environment settings, broker deployment/code/policy/routes/secrets/key inventory, and in-flight operations. If the repaired SHA has no mechanically affected live governance node, emit a validated no-replay receipt. Otherwise authorize and claim the fixed `delivery-governance-broker-upgrade-v1` DAG through the existing protected `ledger-writer` gate. Bind exact expected prestate/target/method/path/request digests; execute one reviewed settings or broker alternative at a time; append request/receipt/post-query; and on partial failure restore exact prior broker version/settings when possible or disable/contain affected operations. Any `ProtectedReleaseTagV1` repair must preserve the exact namespace, rule set, sole Ledger Writer bypass, three-GET actor/admin/ruleset observer contract, canonical create-if-absent tag surface, and closed notes/latest-disabled/no-discussion/empty-assets Release surface; no general read, update/delete/force/upload route, implicit-tag branch, or tag/Release creation is allowed here. Append terminal or containment before proceeding. The DAG cannot create/replace root authority, ledger genesis/App identity, or any bootstrap credential/path.
**Output**: Dependency decision; exact prestate/target graph; authorization/claim/approval/terminal-or-containment commits; main/ledger/tag-ruleset/environment settings, broker route/deployment/key-inventory receipts and post-queries; rollback/disable proof; or no-replay receipt.
**Acceptance**: Root bootstrap remains permanently disabled. No-replay requires exact unaffected live state; every live change is protected, claimed, expected-prestate-bound, least-privilege, fully receipted, rollback-or-containment complete, terminal, and re-queried. Ledger/provider mutation occurs only inside the claimed upgrade DAG and cannot alter root/App/genesis identity or weaken/expand release-tag authority; no tag is created.
...
**What**: Only after Units 65c-65d succeed for the current observed/effective `ReleaseSetPublished`, authorize and claim `release-tag-project-v1` bound to Unit 65d's fresh signed unconsumed `ProtectedReleaseTagSnapshotV1`, deterministic `release-set/<ProductChangeID>/<publicationSHA>` name, exact canonical annotated-tag object bytes/OID, absent-ref prestate, publication/observation/main-pointer/attestation digests, and one exact Release operation ID. The Ledger Writer broker verifies/atomically consumes the pre-snapshot nonce, re-queries absent ref/Release and unchanged latest-release state, mints a bounded token, and creates the annotated tag object and ref with create-if-absent only. Before any Release call it re-queries that exact ref/OID; an absent or mismatched ref contains and the implicit-tag branch is forbidden. It then calls only `POST /repos/spoonjoy/spoonjoy-delivery/releases` with an exact closed payload: canonical `tag_name`; `target_commitish` equal to Unit 64c's exact 40-character publication commit; policy-derived `name`; exact sanitized `body` containing the operation ID plus immutable publication/main-pointer/attestation links and digests; `draft: false`; `prerelease: false`; `generate_release_notes: false`; and `make_latest: "false"`. `discussion_category_name` and every unknown field are omitted. Authorization binds the expected root-inventoried Ledger Writer bot ID/login/type, policy-expected `immutable` value, exact preexisting latest response digest or `404`, no discussion side effect, response/timestamp window, and `assets: []`.

  Record the canonical full `201` response under the pinned media/API version, including `id`, `node_id`, every repository/release/archive URL, `tag_name`, `target_commitish`, `name`, `body`, `draft`, `prerelease`, `immutable`, `created_at`, `published_at`, complete `author`, optional discussion field, and complete asset inventory, plus response Date/request ID and raw-body digest. Require exact tag/name/body/flags, expected bot author, expected immutable value, absent/null discussion, empty assets, operation identity, and timestamps inside the authorized provider window. Re-query the exact ref/object, release-by-tag, release-by-ID, asset inventory, and latest-release endpoint; latest must equal its exact prior response or remain `404`. Lost `201` responses use those same complete normalized queries: exact equality is idempotent success; equal body with wrong tag/target/author/discussion/immutable/latest/timestamp/asset/URL/ID state is collision containment. Enter `AWAITING_RULESET_POST_OBSERVATION`, revoke or contain the write token, and accept only a fresh separately nonced governance-observer post-snapshot from a new complete three-GET identity/permission/ruleset bundle with identical actor/admin identities, ruleset ETag, and normalized ruleset digest, current signer generation, and valid expiry. Consume that nonce, repeat the complete state query, and append terminal. No Release update/delete/upload or tag update/delete/force route exists.
**Output**: Authorization/claim/awaiting-post/terminal-or-containment commits; operation/run/attempt/broker/token identities; signed pre/post snapshot bytes/digests/nonces/expiries and their complete three-response bundle digests with identical ruleset ETag/normalized rules; canonical tag object/ref/name/OID, post-create ref-visibility proof, and create receipt; collision/retry/implicit-tag refusal; exact Release request and complete normalized `201`/GET response fields, Ledger Writer bot identity, provider Date/request IDs, timestamp window, latest-before/after state, null/absent discussion, empty asset inventory, IDs/URLs/raw-body digests; token revocation-or-expiry containment; update/delete/upload/general-route absence; and authoritative post-query.
**Acceptance**: Guard rejects absent/aborted/stale generations, wrong namespace/object/principal, missing/omitted bypass actors, switched viewer/collaborator, non-built-in admin role, stale/skewed/cached/`304` snapshot evidence, stale/replayed/cross-signed nonce, missing/changing ruleset ETag or normalized digest, permissive/drifting rules, existing mismatch, or an absent annotated ref before Release POST. Exactly Ledger Writer creates the absent canonical nested tag once under active `ProtectedReleaseTagV1`, and the exact closed Release request sets notes/latest false and omits discussions; retries prove complete response and provider-state equality. Equal body with wrong author/tag/target/discussion/immutable/latest/timestamp/assets/identity contains; nonempty/duplicate assets and every update/delete/force/upload route fail. The body-only empty-assets GitHub Release identifies the current Unit 64c publication and Unit 65c pointer without becoming latest or creating a discussion; matching fresh privileged pre/post bundles and terminal token handling complete before terminal; no private artifact and no alternate shipment authority.

### ⬜ Unit 67: Post-Shipment Read-Only Closeout
**What**: Only after Units 64c and 65a-66 prove current-generation effective observed publication plus protected-main/tag/Release projections, read-only verify ledger/`PublicationObserved`/main pointer/`ProtectedMainV1`/`ProtectedReleaseTagV1`/tag object/Release/providers/feedback/in-flight state; privately enumerate cleanup paths/handles/caches and publicly record only manifests/digests, source teardown proofs, and artifacts; update Desk/lessons/docs; retire clean task-owned branches/worktrees; notify Slugger; and report a public-safe final inventory.

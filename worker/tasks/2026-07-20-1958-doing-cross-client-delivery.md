...
**Output**: Full local/remote journal chain; signed genesis/object/ref reconciliation; exact-main/CI; distinct App/install/permission/key fingerprint inventories; installation reapproval receipts; RAM-volume/download/import/purge/unmount/local-zero proofs; scoped Cloudflare bootstrap credential revocation or containment; distinct broker challenges; Guardian rules/check dry proof; ledger rules/environment/genesis identities; per-call reconciliation; or exact rollback proof.
**Acceptance**: Live application occurs only after reviewed exact head. Main requires zero human approvals, zero bypass, and exact Guardian checks; journal/ledger genesis match intent and only Ledger Writer bypasses ledger. Exactly one challenged key per App exists only in its broker binding; identities/permissions cannot substitute; no PEM, mounted volume, local/bootstrap credential, or uncertainty remains. Broker endpoints remain root-only; no `RootAuthorityInstalled` yet; every injected crash converges to exact target, inert rollback, or containment.

### ⬜ Unit 12d: Protected Ledger Append Live Proof
**Authority assertion**: `RootAuthorityInstalled` binds both exact App identities, installations, target permissions, broker code/policy/key fingerprints, reviewer-attestation SPKI/generation/revocation contract, Guardian check contract, root-authorized `source-guardian-cutover-v1` graph and target digests, zero-main-bypass/zero-human-approval rules, and Ledger-Writer-only ledger bypass; any missing/substituted authority fails before append.
...
**What**: Permanently forbid Unit 11d/root-bootstrap replay after `RootAuthorityInstalled`. Re-query exact delivery main, authority/root event, normalized repository/rules/environment settings, broker deployment/code/policy/secrets/key inventory, and in-flight operations. If the repaired SHA has no mechanically affected live governance node, emit a validated no-replay receipt. Otherwise authorize and claim the fixed `delivery-governance-broker-upgrade-v1` DAG through the existing protected `ledger-writer` gate. Bind exact expected prestate/target/method/path/request digests; execute one reviewed settings or broker alternative at a time; append request/receipt/post-query; and on partial failure restore exact prior broker version/settings when possible or disable/contain affected operations. Append terminal or containment before proceeding. The DAG cannot create/replace root authority, ledger genesis/App identity, or any bootstrap credential/path.
**Output**: Dependency decision; exact prestate/target graph; authorization/claim/approval/terminal-or-containment commits; settings/broker deployment/key-inventory receipts and post-queries; rollback/disable proof; or no-replay receipt.
**Acceptance**: Root bootstrap remains permanently disabled. No-replay requires exact unaffected live state; every live change is protected, claimed, expected-prestate-bound, least-privilege, fully receipted, rollback-or-containment complete, terminal, and re-queried. Ledger/provider mutation occurs only inside the claimed upgrade DAG and cannot alter root/App/genesis identity.

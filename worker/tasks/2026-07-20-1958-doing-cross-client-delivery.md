...
**What**: Add red fixture tests for every planning object, the `github-environment-ui-bootstrap-v1` and `github-environment-ui-recovery-v1` authorization/claim/receipt unions with deliberately null workflow fields, the installed-gate governance union with required run fields, strict JSON/YAML parsing, duplicate keys, unknown fields, invalid versions/IDs/SHAs/digests/timestamps, empty collections, malformed unions, and schema fixture drift.
...
**What**: Add red semantic tests for authority roles, GitHub actor/run provenance, allowed transitions, monotonic generations, expected-parent CAS, active-generation reservation, pre/post-claim supersession, emergency cancellation, terminal containment, expired claims, direct-child finalization, unauthorized/self-declared appends, bootstrap success/pre-mutation abort/partial-failure rollback-or-roll-forward semantics, drift-recovery authorization, and every race ordering.
...
**What**: Implement `src/authority.ts`, `src/ledger.ts`, `src/transitions.ts`, immutable actor policy `policies/authority-v1.yaml`, exact source-agnostic authority rules for `github-environment-ui-bootstrap-v1` and `github-environment-ui-recovery-v1`, append payload/digest generation, and local history validation for `refs/heads/release-ledger`. Bootstrap success is consumed once per repository/environment identity and pre-mutation abort is the only non-consuming terminal. After any partial mutation, exact-prestate rollback is a nonterminal receipt waypoint under the same active claim, and execution must then roll forward to the originally authorized protected target; reauthorization and cancellation are forbidden. Recovery is permitted only on proven live drift and may restore only the last terminally attested protected policy.
**Output**: Deterministic ledger transition engine.
**Acceptance**: Only protected-workflow/provider-bound actors and legal expected-parent transitions validate; bootstrap cannot be consumed twice, pre-mutation abort does not consume it, any post-mutation rollback remains nonterminal until the same claim reaches protected-target success, recovery cannot alter policy or run without drift, and neither exception authorizes source/provider mutation; shipment is possible only through direct-child `ReleaseSetPublished`.
...
**What**: Implement `src/operation-graph.ts`, `src/receipts.ts`, fixed source-agnostic graphs for `github-environment-ui-bootstrap-v1`, its `github-environment-ui-governance-v1` installed-gate successor, and `github-environment-ui-recovery-v1`, canonical expression resolution, dry-run plans, apply-time revalidation, receipt chaining, and containment planning. Interactive governance nodes are limited to authenticated viewer proof, a matching approved waiting-job grant for the successor, named GitHub environment settings, sanitized UI/API evidence, nonterminal exact-prestate rollback followed by same-claim target roll-forward, and terminal append; recovery's target must equal the last attested policy; provider/source operation node kinds are structurally forbidden.
...
**What**: Implement `src/github.ts`, `src/attestations.ts`, authenticated `gh` adapter, exact dispatch/wait/download/verify, environment review handshake, `environment bootstrap|govern|recover plan|receipt|verify` for the fixed interactive graphs, and artifact-attestation verification. The CLI validates and hashes bounded browser-produced UI evidence; it cannot synthesize actor identity, treat partial state as terminal, change recovery's target, or invoke source/provider operations.
...
**What**: Add red workflow/semantic tests for exact-SHA append workflow, expected-parent update, provider actor/run binding, claim/terminal/cancellation/finalization payloads, singleton environment governance, final source-attestor dispatch plans/artifacts/deadlines, Release Set proof graph, stale evidence, direct-child shipment, aborted finalization, and non-authoritative main/tag projections.
...
**What**: Add red end-to-end CLI tests for init/validate/digest/authorize/dispatch/claim/receipt/evidence/rebaseline/cleanup/finalize/project and `environment bootstrap|govern|recover` commands, stable exit codes/JSON, sample Product Change/Contract/Release Set validation, and docs command/path drift.
...
**What**: Wire the CLI; add `examples/`, `docs/architecture.md`, `docs/authority-and-ledger.md`, `docs/operation-graphs.md`, `docs/evidence-policy.md`, `docs/source-integration.md`, `docs/rollback.md`, and `docs/photo-studio-pilot.md` without secrets/private paths, including bootstrap failure/consumption semantics, exact UI paths, receipt semantics, installed-gate changes, and restore-only drift recovery.
...
**What**: Append authorization and a claim for the versioned `github-environment-ui-bootstrap-v1` exception, re-query the authenticated GitHub viewer as actor `16390116`, capture the existing web `production` environment through REST and the GitHub Settings UI, then use the authenticated UI at `Settings > Environments > production` to set required reviewer `16390116`, allow self-review (`prevent_self_review=false`), turn off `Allow administrators to bypass configured protection rules`, and select `Protected branches only`. Capture every attempt and post-save state. A failure before any UI mutation may append a non-consuming abort. After any partial mutation, the same claim must stay active; an exact-prestate rollback is only a nonterminal recovery waypoint and must be followed by roll-forward to the original target, which is the sole post-mutation terminal. This exception exists only because the target environment cannot approve the operation that first installs its reviewer gate; it grants no source deploy or provider mutation authority.
**Output**: Authorization/claim/success-or-pre-mutation-abort commit; per-attempt actor-viewer response; exact repository/environment IDs; before/after REST responses; before/apply/after screenshot, accessibility-tree, and URL digests; `operation_execution_kind=github_ui_bootstrap_v1`; explicitly null workflow run/attempt fields; governance receipt digest; and authoritative post-query.
**Acceptance**: Receipt binds exact claim/graph/actor/repository/environment and exception version; exact target success consumes the sole bootstrap, exact-prestate pre-mutation abort does not, and any post-mutation path remains under the same claim until target success; UI and API evidence agree on every API-visible field; UI independently proves administrator bypass is off; no workflow or deploy runs.
...
**Output**: Authorization/claim/success-or-pre-mutation-abort commit; per-attempt actor-viewer response; exact repository/environment IDs; before/after REST responses; before/apply/after screenshot, accessibility-tree, and URL digests; `operation_execution_kind=github_ui_bootstrap_v1`; explicitly null workflow run/attempt fields; governance receipt digest; and authoritative post-query.
**Acceptance**: Receipt binds exact claim/graph/actor/repository/environment and exception version; exact target success consumes the sole bootstrap, exact-prestate pre-mutation abort does not, and any post-mutation path remains under the same claim until target success; UI and API evidence agree on every API-visible field; UI independently proves administrator bypass is off; no workflow or TestFlight run starts.
...
**What**: After Unit 63b records a green publish decision, execute Unit 63a's single-use plan as one uninterrupted local orchestration: use authenticated `gh` to dispatch both exact source-owned final-query attestors, wait for and verify their private Cloudflare/D1/R2/ASC state fingerprints, provider audit/watermark metadata where exposed, workflow/source/delivery identities, and GitHub-attested artifacts; replace freshness-bound evidence; recompile and leak-scan the complete Release Set; then start the protected append workflow within 120 seconds of the later attestor completion. Before choosing a terminal, that workflow re-queries source main/check identities, source mutation-run inventories since each attestor query, public Worker/runtime digests, evidence expiry, exact artifact identities, nonce reuse, and ledger parent. If every value is unchanged, expected-parent append the set as the direct child of Unit 61. If any dispatch/query/verification/compile/deadline/pre-append check fails, append `FinalizationAborted` instead and return to reviewer-gated repair/replay at Unit 60a.
**Output**: Final attestor run/attempt/artifact/attestation IDs and provider query timestamps; refreshed Release Set/query/leak digests; append run ID; ledger parent; authoritative `ReleaseSetPublished` or `FinalizationAborted` commit; actor/workflow proof; and post-query.
**Acceptance**: Private provider state is re-queried inside Unit 64 at exact source/delivery identities; the 120-second bound is enforced from the later query; the append workflow emits exactly one direct-child terminal; only a fully matching final snapshot with no detected later source/public drift can produce the sole shipment event, while any uncertainty atomically aborts before repair.
...
- The first-install `github-environment-ui-bootstrap-v1` and restore-only `github-environment-ui-recovery-v1` operations are the only operations exempt from their target environment's approval gate: both require protected ledger authorization/claim, authenticated actor-ID proof, bounded UI changes, UI/API evidence, and unambiguous terminal state; neither can invoke source/provider mutation code. Bootstrap installs a new gate once; recovery requires proven drift and can only restore the last attested policy. All policy changes use the installed gate.

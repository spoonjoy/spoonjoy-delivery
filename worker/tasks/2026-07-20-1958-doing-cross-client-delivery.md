...
**What**: Add red semantic tests for authority roles, GitHub actor/run provenance, dedicated ledger GitHub App/installation identity, transitions/generations/CAS, supervisor-envelope provisional start/challenge/provision/abort/rotation/retirement with distinct repository identities, exact Started-commit/key/fingerprint preservation, substitution rejection, and active-run retention, durable recovery registration/expiry, resolver gating across reservation/claim/pending publication, coordinator/recovery races, direct-child finalization plus mandatory observation effectiveness proof, unauthorized App or ordinary `GITHUB_TOKEN` appends, receiver/bootstrap/drift recovery, and every ordering.
...
**What**: Implement `src/operation-graph.ts`, `src/receipts.ts`, deterministic rollback-target selection/setup planning, fixed source-agnostic governance, repository-distinct supervisor-envelope start/provisional-challenge/provision-or-abort/rotate/retire graphs, a post-root delivery-governance/broker-upgrade graph, canonical expression resolution, dry-run/revalidation/receipt/containment, and `ledger-root-bootstrap-v1`. Key graphs bind Cloudflare secret prestate, provisional/active event generation, canonical X25519 key/fingerprint, exact Started commit, byte-identical repository/generation/key/fingerprint in Provisioned, exact challenge/ordinary OIDC allowlists, active-run retention, bootstrap credential, local-zero, delete/contain alternatives, and terminal. Post-root upgrade binds exact root event and live settings/broker prestate, permits only reviewed deltas/rollback/disable containment, and forbids root/App/genesis changes. The root graph binds authenticated operator, exact repository/broker/App/ruleset/environment prestate and code digests; permits only install, exact-target resume, or exact-prestate rollback; emits one App-authenticated `RootAuthorityInstalled`; verifies setup credential/path removal; and thereafter rejects every root-bootstrap invocation. Candidate selection retains documented total-order/fail-closed rules.
...
**What**: Add red tests for CI, pinned Actions, attestations, `ProtectedMainV1`/`ProtectedLedgerV1`, exact root-intent signature/digest domains, signed genesis/object/ref transport and later fast-forward-only journal appends, root App/bootstrap-credential key custody, supervisor-envelope provision/rotation/one-use OIDC unwrap, canonical X25519 key/fingerprint, Started-to-Provisioned commit/key continuity, resume/rollback/kill switch, sole App bypass, and broker identity/actor/transition enforcement. Model `token_mint_started/token_issued/token_revoke_started`, conservative expiry containment, bounded-byte mailbox, prepare/commit/status-observe, and every crash. Live negatives include wrong positive check App IDs, extra ledger bypass principals, legal OIDC with illegal event, unauthorized rerun/unwrap/replay, wrong repository/workflow/job/key generation/capsule, Provisioned key/fingerprint/start-commit substitution, provisional challenge use on an ordinary endpoint, caller-created Git objects, substituted/oversized package bytes, orphan/duplicate genesis objects, ref-create conflict, lost App-key response, duplicate App/envelope keys, secret-write uncertainty, local key residue, and unreconciled bootstrap-token revocation.
...
**What**: Add SHA-pinned CI/resolver. Define `ProtectedMainV1` with repository-specific `{context, expected App ID}` checks, and `ProtectedLedgerV1` with exactly the policy-pinned ledger App bypass and no ordinary writer path; both normalize/stability-check all layers. Implement serialized broker identity/actor/transition validation plus distinct web/native envelope-key generations and one-use unwrap. Canonical recipient key is raw RFC 7748 32-byte X25519 u-coordinate in base64url without padding; fingerprint is lowercase-hex SHA-256 of those bytes. Capsule encryption is RFC 9180 base-mode HPKE suite `{KEM=X25519-HKDF-SHA256(0x0020),KDF=HKDF-SHA256(0x0001),AEAD=ChaCha20Poly1305(0x0003)}` with fresh ephemeral KEM key and sequence zero. Strict RFC 8785 envelope is exactly `{version:1,suite,enc,ciphertext,payloadDigest,context}` with 32-byte `enc` and base64url-no-padding fields. `info` is `UTF8("spoonjoy-supervisor-capsule-v1") || 0x00 || RFC8785({repository,workflow,keyGeneration})`; AAD binds owner/repository/workflow/ref/SHA/run/attempt/checkRun/job/environment/claim/generation/nonce/expiry/payloadDigest/signerPublicKey. `payloadDigest` hashes canonical plaintext; claim separately binds final-envelope SHA-256. Reject reused `enc`/nonce/context, caller AEAD nonce, and noncanonical encoding. Normal unwrap requires exact active Provisioned event whose `startedCommit`, generation, canonical key, and fingerprint equal its Started event byte-for-byte; validate OIDC/JTI, consume nonce before decrypt, return signer bytes once, and reject replay/retired/contained. Provisional endpoint requires exact Started event/claim/generation/key/fingerprint/challenge job/nonce, decrypts only fixed challenge once, and cannot unwrap signer capsules; challenge receipt binds start commit/key/fingerprint and only an exactly preserving Provisioned transition activates ordinary unwrap. Timeout/crash deletes binding and appends Aborted or contains all unwraps. App key never leaves broker; envelope private key exists first only in the isolated provisioning context, then broker storage, and local purge is mandatory before activation completes. Model append state independently from credential lease; pre-mint containment, returned expiry/fingerprint persistence, `finally` revocation `204` or expiry containment, tokenless `H`, fresh-token abort CAS, and `cas_uncertain` persistence remain mandatory.
...
**What**: Authorize and claim one `web-supervisor-envelope-key-provision-v1` operation under the root policy's fixed repository/workflow/key-lifecycle graph. Generate a distinct X25519 keypair inside an enumerable isolated ephemeral private context before provider mutation; encode the public key as the raw RFC 7748 32-byte u-coordinate in base64url without padding and derive its lowercase-hex SHA-256 fingerprint. Broker-append `SupervisorEnvelopeKeyProvisioningStarted` binding claim, next generation, exact canonical public key/fingerprint, expected web challenge workflow/job, one nonce, and expiry. Through a one-use scoped Cloudflare credential stream the matching private key into a provisional broker binding. Dispatch/approve only the bound disposable web challenge job; the provisional endpoint requires the exact Started commit/event/OIDC/claim/generation/key/fingerprint/nonce and decrypts only the fixed test payload once. Its receipt binds that same Started commit/key/fingerprint. After success, broker-append `SupervisorEnvelopeKeyProvisioned` referencing the exact Started commit and preserving repository/generation/public key/fingerprint byte-for-byte, prove local private-context destruction/fsync, then activate ordinary unwrap. Crash before start only purges local context; timeout/crash/failure after start deletes binding and appends `ProvisioningAborted`; uncertain deletion contains all unwraps. Inventory generation, revoke bootstrap credential, and journal every partial failure. Rotation provisions N+1 provisionally, keeps N decrypt-only until bound runs terminate, then retires/deletes N.
**Output**: Authorization/claim/start/provision-or-abort/retire/terminal-or-containment commits; exact Started commit; canonical public-key bytes/event/policy digests; provisional/active binding generation/fingerprint and equality proof; scoped credential creation/revocation; exact challenge/receipt; local-zero/key inventory; rotation/deletion/partial-failure receipts; and post-query.
**Acceptance**: Exactly one active web generation preserves its Started repository/generation/key/fingerprint exactly, is policy-pinned, and is usable only by broker-validated web OIDC/claim/capsule identity; private unwrap key never enters GitHub Actions, arguments, logs, or artifacts. Bootstrap credential and local private context are provably absent before activation or all unwraps remain contained; no deploy/provider mutation occurs.

### ⬜ Unit 27d2: Web Production Environment Protected Proof
**What**: Authorize and claim the merged web workflow's read-only `verify-environment-governance` operation. Its credential-free ordinary preflight re-queries the exact active Provisioned event and Started commit, requires byte-identical repository/generation/canonical key/fingerprint, creates a fresh ordinary one-use signer capsule bound to this exact run/claim, and explicitly submits a prior provisional challenge capsule/generation to the ordinary endpoint as a required rejection. Dispatch at exact main, prove its sole `production` job enters `waiting`, re-query approver actor `16390116`, approve that exact waiting job, verify only the fresh ordinary capsule unwrap reaches the supervisor through the ordinary endpoint, and append its sanitized verification receipt plus terminal or containment.
**Output**: Authorization/claim/terminal-or-containment commits; workflow/run/attempt/job/environment IDs; exact Started/Provisioned commit and key-equality proof; fresh ordinary capsule/endpoint receipt; provisional-capsule rejection receipt; waiting and approval API responses; actor proof; verification receipt digest; and authoritative post-query.
**Acceptance**: Exactly one claimed job waits and is approved by actor `16390116`; exact Started-to-Provisioned key continuity validates, the fresh run-bound ordinary capsule unwraps once, and the prior provisional challenge capsule/generation is rejected by the ordinary endpoint. It validates current environment settings without access to Cloudflare/D1 secrets or deploy commands; terminal-or-containment appended; no deploy occurs.
...
**What**: Authorize and claim one `native-supervisor-envelope-key-provision-v1` operation under the root policy's fixed repository/workflow/key-lifecycle graph. Generate a distinct X25519 keypair inside an enumerable isolated ephemeral private context before provider mutation; encode the public key as the raw RFC 7748 32-byte u-coordinate in base64url without padding and derive its lowercase-hex SHA-256 fingerprint. Broker-append `SupervisorEnvelopeKeyProvisioningStarted` binding claim, next generation, exact canonical public key/fingerprint, expected native challenge workflow/job, one nonce, and expiry. Through a separate one-use scoped Cloudflare credential stream the matching private key into a provisional broker binding. Dispatch/approve only the bound disposable native challenge job; the provisional endpoint requires the exact Started commit/event/OIDC/claim/generation/key/fingerprint/nonce and decrypts only the fixed test payload once. Its receipt binds that same Started commit/key/fingerprint. After success, broker-append `SupervisorEnvelopeKeyProvisioned` referencing the exact Started commit and preserving repository/generation/public key/fingerprint byte-for-byte, prove local private-context destruction/fsync, then activate ordinary unwrap. Crash before start only purges local context; timeout/crash/failure after start deletes binding and appends `ProvisioningAborted`; uncertain deletion contains all unwraps. Inventory generation, revoke bootstrap credential, and journal every partial failure. Rotation provisions N+1 provisionally, keeps N decrypt-only until bound runs terminate, then retires/deletes N.
**Output**: Authorization/claim/start/provision-or-abort/retire/terminal-or-containment commits; exact Started commit; canonical public-key bytes/event/policy digests; provisional/active binding generation/fingerprint and equality proof; scoped credential creation/revocation; exact challenge/receipt; local-zero/key inventory; rotation/deletion/partial-failure receipts; and post-query.
**Acceptance**: Exactly one active native generation, distinct from web, preserves its Started repository/generation/key/fingerprint exactly, is policy-pinned, and is usable only by broker-validated native OIDC/claim/capsule identity; private unwrap key never enters GitHub Actions, arguments, logs, or artifacts. Bootstrap credential and local private context are provably absent before activation or all unwraps remain contained; no TestFlight/provider mutation occurs.

### ⬜ Unit 27e2: Native Internal-TestFlight Environment Protected Proof
**What**: Authorize and claim the merged native workflow's read-only `verify-environment-governance` operation. Its credential-free ordinary preflight re-queries the exact active Provisioned event and Started commit, requires byte-identical repository/generation/canonical key/fingerprint, creates a fresh ordinary one-use signer capsule bound to this exact run/claim, and explicitly submits a prior provisional challenge capsule/generation to the ordinary endpoint as a required rejection. Dispatch at exact main, prove its sole `internal-testflight` job enters `waiting`, re-query approver actor `16390116`, approve that exact waiting job, verify only the fresh ordinary capsule unwrap reaches the supervisor through the ordinary endpoint, and append its sanitized verification receipt plus terminal or containment.
**Output**: Authorization/claim/terminal-or-containment commits; workflow/run/attempt/job/environment IDs; exact Started/Provisioned commit and key-equality proof; fresh ordinary capsule/endpoint receipt; provisional-capsule rejection receipt; waiting and approval API responses; actor proof; verification receipt digest; and authoritative post-query.
...
- 2026-07-21 04:18: Tinfoil Round 12 bound canonical X25519 public-key bytes and fingerprint from Started through exact-commit Provisioned activation, required local private-context purge before activation, and replaced disposable challenge reuse in governance proof with a fresh ordinary run-bound capsule plus mandatory provisional-endpoint rejection.
- 2026-07-21 04:21: Froze the repaired planning and doing documents as `READY_FOR_EXECUTION`; the final exact-tree Tinfoil and Stranger With Candy scrutiny sequence remains the execution gate.

...
**What**: In increasing fM order, create each dedicated delivery repair branch fresh from then-current exact main, complete it through TDD, open its PR, and obtain protected CI/terminal review without merge. Validate no-PR when no-op.
**Output**: Per-finding branch/PR/head/run/review IDs, or no-PR receipt.
**Acceptance**: Every exact head green with no unresolved/in-flight check; `repair-iN-rK` untouched; no merge or shipment.

### ⬜ Unit 60b1c: Delivery Repair Merge
**What**: In increasing fM order, merge each reviewed code PR, verify exact main/CI, then retire its clean code branch/worktree; do not merge or rewrite the records branch. Validate no-merge when no-op.
**Output**: Per-finding merge SHA/exact-main CI/cleanup receipt or no-merge receipt.
**Acceptance**: Delivery main green and covers every finding; all code worktrees retired; records branch still has identical head/K; no shipment.
...
**Acceptance**: Pass number is monotonic; all nodes present once; latest pass is authoritative; durable no-replay has byte-equal evidence; refresh/replay propagate deterministically; no stale pass can drive a unit; no provider mutation occurs.
...
**Acceptance**: Every renewable class is queried exactly once per provider/account scope; all calls read-only; durable proof remains unchanged; matching state converges without mutation replay; mismatches fail closed through a new numbered evaluation.
...
**What**: Immediately before Unit 61, run the Unit 60c59b read-only renewable-observation batch from protected Unit 60f3 records, then run a new numbered Unit 60c29f evaluation with fresh GitHub server time. Durable proofs remain valid by exact identity; renewable predicates must match current state. A mismatch/unknown increments repair iteration and loops to Unit 60a for mechanically selected repair/replay and a new records checkpoint; age alone causes another read-only query, not upload/hardware/mutation replay.

...
**What**: Add red workflow/semantic tests for append/authority/recovery/resolver/finalization plus executable Linux/macOS supervisor isolation: distinct child UID/session, peer-credential-gated signer service, no inherited signer FD/socket/mount, ptrace/task-port/privilege denial, read-only host mounts, cgroup/process-group exhaustion, cancellation recovery, and fail-closed unsupported host. Cover staged tombstone through lock-gated post-fsync `PROMOTED`, pre-CAS/observation races, pending publication, and projections.
...
**What**: Cover every finalization race plus hostile child signer invocation, `/proc`/task-port access, inherited descriptors, advisory-lock bypass, forked descendants, signals/cancellation, UID/peer-credential mismatch, cgroup/process-group exhaustion, unsupported isolation, and crashes/verifier races before/after pending fsync, key destruction, rename, directory fsync, `PROMOTED`, observation, and projection.
...
**What**: Add red tests for exact identities/provider queries and Unit 8's Linux supervisor profile: distinct UID/cgroup, peer-credential signer ACL, no signer FD/socket/host mount, ptrace/privilege denial, read-only root/private tmpfs, descendant exhaustion, cancellation recovery, and separate no-secret verifier. Include hostile signer/`/proc`/FD/lock/fork/signal children, teardown/residue, evidence, expiry, and wrong run/ref/event.
...
**What**: Add red tests for every ASC operation/receipt/failure and Unit 8 Linux supervisor profile: distinct UID/cgroup, peer-credential signer ACL, no signer capability/mount, ptrace/privilege denial, descendant exhaustion, hostile child/fork/signal/cancellation cases, and separate no-secret teardown/object verifier.
...
**What**: Add red tests for exact identity/ASC state/windows and Unit 8 Linux supervisor profile: distinct UID/cgroup, peer-credential signer ACL, no signer capability/mount, ptrace/privilege denial, descendant exhaustion, hostile child/fork/signal/cancellation cases, and separate no-secret teardown/object verifier. Reject absent teardown, objects, expiry, and wrong run/ref.
...
**What**: Before private query, initialize enumerable `0700` root and fsynced WAL `ALLOCATING -> OPEN -> SEALED -> PURGING -> PURGED -> PROMOTED`. Run exact supervisor outside the secret-bearing writer's UID/session. Linux uses root-owned supervisor socket with `SO_PEERCRED`, `FD_CLOEXEC`, non-dumpable/no-new-privileges child, no host socket/mount, read-only root plus private tmpfs, seccomp and dedicated cgroup killed to empty. macOS uses root launchd service/socket with local peer credentials, separate unprivileged UID, denied task-port/ptrace/privilege escalation, no inherited descriptor/mount, and supervised process group killed/enumerated empty. Signer authorizes only its bound context/state after peer checks. Unsupported boundary is `BLOCKED_HUMAN`, never clean. Writers share-lock and append-fsync manifest; supervisor exclusively purges/restarts orphans.
**Output**: Context/public-key/policy and Linux/macOS isolation-profile digests, supervisor/child UID/session/peer proofs, empty descendant-group proof, fsynced WAL/lock/restart evidence, and pending/final receipt fixtures.
**Acceptance**: Supervisor boundary is executable and tested on active platform before writes; child cannot reach signer, host process APIs, privilege, locks, or surviving descendants. Cancellation/restart converges or yields no receipt. Crash injection leaves discoverable state or non-authoritative `.pending`; no final tombstone validates without lock-gated matching `PROMOTED`. Partial setup routes to Unit 64b; no mutation.

...
**What**: Add red tests for CI, pinned Actions, attestations, advisory gates, protected refs, root-bootstrap/resume/rollback/self-disable, sole App bypass, and an OIDC append broker rejecting wrong repository/owner/actor/environment/workflow-ref/workflow-SHA/run/attempt/audience/nonce/event/expected-parent, replay, expiry, or noncanonical request. Prove installation tokens never enter Actions/logs/artifacts and the broker can perform only the one requested CAS append. Test resolver, reviewers, minimal `id-token: write`, and live negative principals/reentry.
...
**What**: Add SHA-pinned CI plus an exact-SHA read-only resolver required by main and every ledger mutation. During active reservation/claim/pending publication it fails merges/unrelated transitions and passes only bound coordinator/recovery operations. Implement a Cloudflare append broker whose secret-held App key and internally minted <=60-second contents-only installation token never enter Actions. After `ledger-writer` approval it verifies GitHub OIDC issuer/signature/audience plus repository/owner/actor/environment/workflow-ref/workflow-SHA/run/attempt, canonical event bytes, expected parent, and single-use nonce; it creates the exact Git object/ref CAS itself and returns a signed receipt. Exact jobs alone request `id-token: write`. Bind broker deployment digest and identities into policy/claims. Add settings/apply plan and fixtures.
...
**What**: Cover missing/bypassed resolver checks, registration/claim/provider query failure, OIDC signature/JWKS rotation/audience/claim/replay/nonce/expiry/token-scope/broker-secret failures, unpinned actions, permission escalation, wrong reviewer/bypass, failed API reads, pagination, and settings/broker drift.
**Output**: Coverage and warning logs.
**Acceptance**: 100% coverage, all delivery gates green, zero warnings.

### ⬜ Unit 11d: Delivery Repository Settings - Apply and Verify
**What**: Begin the reviewed `ledger-root-bootstrap-v1` ceremony from captured GitHub/Cloudflare prestate: create/configure the least-privilege App/install; deploy the exact token broker disabled for issuance with App key only as Cloudflare secret; apply protected main/ledger, resolver/admin/rules, sole App bypass, read-only workflow defaults, and reviewed `ledger-writer` environment. Persist a bounded pending-bootstrap receipt that blocks unrelated delivery/source/provider mutations. On interruption only exact-target resume or exact-prestate rollback is legal. Do not issue a ledger token or append before the workflow reaches protected main.
**Output**: Before/apply/after JSON, pending-bootstrap receipt, rollback/resume proof, App/broker/ruleset/environment digests, and settings-verifier report.
**Acceptance**: Infrastructure matches the exact reviewed target, broker issuance remains disabled, no ledger append occurs, pending bootstrap blocks unrelated work, and rollback remains possible until Unit 12d.
...
**What**: Merge the protected PR, verify exact delivery main/CI, preserve the coordinating worktree, and rotate it onto clean `worker/cross-client-delivery-records-r0` based on verified main for Units 13-29e. This branch is only a PR staging branch and is never itself treated as protected handoff evidence.
...
**What**: From exact merged delivery main, complete the pending root ceremony: enable broker only for the exact merged install workflow, have it append `RootAuthorityInstalled` without returning its token, verify App/broker/ruleset/environment digests, then remove and prove absence of every bootstrap credential/path and prove a second bootstrap fails. Next prove direct update, ordinary write-enabled `GITHUB_TOKEN`, alternate App, wrong workflow/environment, and token-exfiltration attempts fail while a disposable non-shipping broker append succeeds; terminally contain it through ledger semantics.
**Output**: Root event/run/commit, bootstrap-disable and negative-reentry proof; fixture/containment commits; App/installation/environment/workflow/broker identities; negative-principal rejections; ruleset/post-query.
**Acceptance**: Root authority is installed once by exact protected main and setup is irreversibly disabled at application level; only policy-bound App tokens can append; fixture is non-shipping and contained; no product/provider mutation.

### ⬜ Unit 13a0: Cross-Task Handoff Contract Interoperability
**What**: Without touching source/provider state, treat release-owner commit `86b30da0b537d768761f8236bf2a423400395931` as a nonterminal predecessor and require its upstream-owned successor to replace unprotected `worker/audit-release-train`/`records-r0` constants with each repository's genuinely protected `main`. Verify canonical outbound/receiver schemas, verifier, adversarial checker, consistent paths, acyclic binding to protected `ReceiverAcknowledged`, and live effective GitHub rules for both main refs. Payload forbids its own containing commits; verifier arguments/query results provide external reachability envelopes. This remains prerequisite repair only, never terminal handoff.
**Output**: Exact upstream successor commit/PR/protected-main reachability, changed path inventory, tests, schema/verifier digests, live main-ruleset evidence for both repositories, and coordination receipt.
**Acceptance**: Upstream owner still owns source; verifier constants and tests require `refs/heads/main` for both acknowledgment copies; one canonical outbound filename exists; receiver schema binds outbound commit/digest, ledger commit/payload, exact protected fields, and receiver IDs; verifier re-queries containing protected-main commits and exact ack bytes. Branch-only/unprotected copies and self-reference fail; tests pass with zero warnings; no source/provider mutation by this task.
...
**What**: Fresh-review and merge Unit 13a2 through protected delivery `main`, verify exact-main CI and effective ruleset, then re-query the containing main commit and exact acknowledgment bytes. Send protected-main/ledger locators to the release owner.
**Output**: Review verdict, PR/merge/main SHA, exact-main run IDs, ruleset query, remote ack bytes/digest, and coordination receipt.
**Acceptance**: Delivery `main` is genuinely protected and contains the exact ack digest; branch-only evidence is rejected; zero in-flight delivery merge/deploy; no source/provider mutation.

### ⬜ Unit 13a3: Upstream Two-Sided Handoff Verification
**What**: Wait for the release owner to ingest the exact Unit 13a2a protected-main copy, commit byte-identical `receiver-ack.json` in its canonical task tree, merge it through protected native `main`, and run the repaired verifier against both protected-main containing commits plus `release-ledger`. Independently rerun delivery `handoff verify` against both remotes and effective main rulesets.
...
**Acceptance**: Both verifiers agree byte-for-byte; outbound and ledger commits are payload-bound; both containing commits are on genuinely protected `main` and contain the exact ack digest without appearing inside it. All are remote-reachable and stable; upstream explicitly releases ownership only now; mismatch/unprotected reachability leaves ownership upstream and blocks Unit 13b.
...
**What**: Dispatch the exact protected coordinator after green Unit 60f4. First CAS-append `FinalizationRecoveryRegistered` with exact recovery SHA/policy, predecessor, local context public key, every source preflight signer public-key/supervisor/policy/capsule/run/nonce digest, deadline, and expiry. Any later exact recovery may consume it and is structurally abort-only. Then CAS-append bound claim and watch. Original may publish/abort; recovery/sweeper may expire an unclaimed registration, abort a dead/overdue claim, or observe an already-appended publication. Active-state resolver rejects every merge/unrelated transition while allowing only exact bound coordinator/recovery operations.
...
**What**: When Unit 63 is green, compile a single-use plan bound to registration/claim/coordinator/recovery/deadline, local and source signer-bootstrap identities, nonce, exact source/workflow identities, purge prerequisite, and honest timing contract. Strict-parse HTTP `Date` as IMF-fixdate GMT. Per provider define `S/C/R`; require `S <= C`, `C-S <=120000`, `-5000 <= R-C <=5000`. Define `E` as `GET /repos/spoonjoy/spoonjoy-delivery/git/ref/heads/release-ledger?nonce=<single-use>` with `Cache-Control: no-cache, no-store`, `Pragma: no-cache`, pinned API version, status 200, matching exact claim head, no `304`, and `Age` absent/zero plus no cache-hit marker. Record monotonic `e0/e1`, require duration `e1-e0 <=10000ms`; prebuild the commit object before `E`, then require ref-update dispatch `d-e1 <=5000ms` and a 15000ms append timeout. Define `H` as the same uniquely noncached exact-ref query started within 5000ms after append success/error/timeout, duration at most 10000ms, with at most three bounded attempts before abort-only recovery. Explicitly forbid interpreting `E` or `H` as an atomic snapshot or provider age at CAS.
**Output**: Validated plan/digest/nonce, registration/coordinator/recovery/context/source-bootstrap/deadline bindings, purge requirements, exact endpoint/header/status/cache/monotonic/deadline contracts for `E`, dispatch, append, and `H`, and green/skip report.
...
**What**: After Unit 63b green and while original coordinator is waiting, recovery is registered, and claim is head, dispatch source final-query attestors using the exact pre-bound signer bootstraps. Verify each teardown signature only against its claim-bound public key plus supervisor/policy/capsule/run/nonce digests; reject alternate/self-signed keys. Verify private Worker/D1/R2/Pages/ASC fingerprints, `S/C/R`, runner manifests, and post-run artifact/cache inventories; recompile/leak-scan and stage one sanitized package. Do not append a terminal. Missing/cancelled/unverifiable cleanup signals original or abort-only recovery, then Unit 64b.
...
**What**: After green evidence and `publication_clean`, ingest package, verify every source receipt against its claim-bound bootstrap, prebuild the candidate commit, and execute Unit 63a's exact `E` query. Parse `E`; for each `T` in `{S,C,R}`, fail below `-5000`, clamp permitted negative skew to zero, and require normalized `E-T <=120000`. Require `e1-e0 <=10000ms`, dispatch CAS update within 5000ms, and enforce its 15000ms timeout. Record response Date/timing `F` without claiming provider age at CAS. After success/loss/ambiguity, start exact `H` observation within 5000ms and enforce its 10000ms/three-attempt bounds. Candidate ref emits non-ledger protected `PublicationObserved`; unchanged claim retries only within plan/deadline or aborts; other head follows legal race. Recovery may observe or abort, never publish.
**Output**: Registration/coordinator/recovery/evidence and source-bootstrap IDs; cleanup attestations; provider `S/C/R/E`; exact endpoint/header/cache evidence; `e0/e1/d/F/H` monotonic and HTTP timing; pending/effective publication or abort commit; `PublicationObserved`; race/post-query proof.
**Acceptance**: `E` proves only prompt dispatch after one bounded pre-CAS observation. Missing/cached/late/unparsable `E`, `e1-e0 >10000ms`, dispatch gap over 5000ms, append timeout, late/failed `H`, invalid bootstrap/cleanup, or drift fails closed and resolves through bounded retry/abort. No pending publication unlocks shipment or projections. Only observed direct-child `ReleaseSetPublished` ships; no distributed-lock or actual-CAS-age claim.

...
**Release-tag authority**: Red tests must model `ProtectedReleaseTagV1` over recursive `refs/tags/release-set/**/*`, prove the exact two-level canonical Unit 66 name matches under GitHub `FNM_PATHNAME`, exactly one Ledger Writer Integration bypass, creation/update/deletion/non-fast-forward rules, and canonical create-if-absent tag events. Add `ProtectedReleaseTagSnapshotV1` tests for distinct root-anchored governance signer, operator actor/repository ADMIN, exact GET-only endpoint, full bypass payload, ETag/Date/request ID, nonce/expiry/one-use, matching pre/post snapshots, missing/unstable version indicator, wrong principal/pattern/object, collision/lost-response reconciliation, protection drift, and absolute denial of observer or broker mutation methods.
...
**Release-tag graph**: Add fictional-provider red coverage for a root-bound namespace-ruleset install/reconcile graph, root-authorized GET-only `release-tag-ruleset-observe-v1`, and separately claimed `release-tag-project-v1` create-if-absent graph. Cover omitted bypass actors, missing/changing ETag, stale/replayed/cross-signed snapshots, nonce/expiry, pre/post mismatch, every collision/response-loss/stale-ruleset/alternate principal/object, and forbidden observer/broker update/delete path.
...
**Release-tag graph**: Implement root-install, local GET-only observation, and projection graphs. The observer requires authenticated actor `16390116` with repository `ADMIN`, exact no-cache ruleset GET, complete bypass actors, and stable ETag/Date/request ID; it emits one-use governance-key-signed `ProtectedReleaseTagSnapshotV1` and cannot mutate. Projection binds a fresh pre-snapshot, allows only Ledger Writer broker creation of one canonical annotated tag/ref from an exact observed publication, then waits for and consumes a fresh post-snapshot with identical ETag/normalized digest before terminal. Reconcile exact tag/release equality on retry; mismatched existing state contains and no graph can update/delete a tag or upload a Release asset.
...
**Release-tag protection**: Red settings/workflow tests include exact active `ProtectedReleaseTagV1` namespace/rules, sole Ledger Writer Integration bypass, canonical create-if-absent broker route, forbidden update/delete/force/arbitrary-name routes, and distinct governance-observer key/route/snapshot. Test full bypass visibility only under operator ADMIN GET, ETag/Date/request ID, one-use pre/post snapshot equality, omitted bypass actors, missing/changed ETag, cross-key/replay/expiry, normalized-rule drift, lost response, wrong-existing-ref/release collision, exact body-only Release fields/links, and mandatory empty asset inventory.
...
**Release-tag authority**: Normalize/verify active tag rulesets independently from branch/ledger rules. `ProtectedReleaseTagV1` matches only recursive `refs/tags/release-set/**/*`, proves the canonical two-level tag is included under GitHub `FNM_PATHNAME`, requires creation/update/deletion/non-fast-forward restrictions, and exactly one always-bypass Ledger Writer Integration. Keep Administration permission away from Ledger Writer. A distinct root-authorized local governance observer authenticates operator `16390116` with repository `ADMIN`, permits only exact no-cache ruleset GET, requires full bypass actors and stable ETag/Date/request ID, and signs canonical one-use `ProtectedReleaseTagSnapshotV1`. Its broker validates `release-tag-project-v1` against fresh matching pre/post snapshots, creates exact annotated object/ref and body-only GitHub Release only when absent, requires an empty asset inventory, and exposes no update/delete/force/general-tag/release-asset endpoint.
...
**What**: Freeze the exact remote-reachable delivery commit containing the distinct Ledger Writer and Delivery Guardian identity-registration DAGs and complete captured-request/crash/reconciliation tests. Generate two distinct non-exportable P-256 keys in Secure Enclave under separate access-control tags isolated from root/App/envelope keys: reviewer attestation and governance observation. Export only each RFC 5480 SPKI/fingerprint/generation, challenge-sign its disjoint domain, and record local inventory/non-substitution. The governance observer implementation authenticates only as operator actor `16390116`, requires repository `ADMIN`, allows only exact no-cache ruleset GET, requires full bypass actors and stable ETag/Date/request ID, canonicalizes/signs one-use `ProtectedReleaseTagSnapshotV1`, and rejects all mutation/general API routes. Dispatch two distinct fresh hostile reviewer tasks with frozen prompt/lens and target digests. Canonicalize `ReviewerEnvelopeV1`, sign it through the reviewer key service, and verify signature, target, task/model/verdict/evidence, nonce, expiry, and local one-use consumption before any provider mutation. Run full local gates, repair every BLOCKER/MAJOR, and repeat until the exact tree is clean.
**Output**: Exact commit/tree, full gate logs, two fresh reviewer task/model/verdict/evidence digests; reviewer and governance-observer SPKI/fingerprint/generation/challenge/workflow-inaccessibility/non-substitution proofs; exact GET-only observer route and `ProtectedReleaseTagSnapshotV1` golden vector; repair commits; canonical signed no-mutation reviewer envelope/digest/nonce/expiry; one-use receipts; and clean provider pre-query.
...
**Tag-governance binding**: Root intent must bind exact recursive `ProtectedReleaseTagV1` name/target/include/exclude/rule/bypass digests, Ledger Writer App ID, `release-tag-project-v1` create-only broker contract, canonical tag schema/domain, governance-observer SPKI/generation/revocation and GET-only route, canonical `ProtectedReleaseTagSnapshotV1` domain/schema/nonce/expiry/ETag contract, and rollback/removal behavior before any live rule exists.
...
**Release-tag live governance**: In the same reviewed root journal, install/reconcile active `ProtectedReleaseTagV1` for recursive `refs/tags/release-set/**/*` with creation/update/deletion/non-fast-forward rules and exactly the Ledger Writer Integration as sole always-bypass principal. Prove the exact canonical two-level example matches under GitHub `FNM_PATHNAME`. Bind/challenge the distinct Unit 11c0 governance-observer SPKI/generation without exporting its private key; through authenticated operator actor `16390116` and repository `ADMIN`, execute only exact no-cache ruleset GET, require full bypass actors plus ETag/Date/request ID, and sign/verify one `ProtectedReleaseTagSnapshotV1`. Verify normalized digest/ETag stability around an authorized broker dry proof; ordinary user/token/Guardian/wrong App creation, observer mutation, and every update/delete/upload route fail. No tag or Release is created.
...
**What**: Before any tag authorization, use root-authorized local `release-tag-ruleset-observe-v1` to independently query and normalize the root-installed active `ProtectedReleaseTagV1` and its root-policy/event bindings. Authenticate exact operator actor `16390116`, require repository `ADMIN`, execute only the exact no-cache ruleset GET, and fail unless GitHub returns complete bypass actors plus nonempty ETag/Date/request ID. Require recursive `refs/tags/release-set/**/*`, prove the exact canonical two-level future name matches under GitHub `FNM_PATHNAME`, creation/update/deletion/non-fast-forward rules, and exactly the Ledger Writer Integration as sole always-bypass actor. Canonicalize/sign a one-use `ProtectedReleaseTagSnapshotV1` under the root-anchored governance-observer SPKI/generation with nonce and short expiry; repeat the no-cache GET before output and require identical ETag and normalized digest. Verify no current target tag/ref/Release collision, body-only Release contract with empty asset set, and broker surfaces containing only canonical create-if-absent with no update/delete/force/general-name/upload route. Run ordinary-user/token/Guardian/wrong-App create, observer mutation, Ledger-Writer update/delete, and Release-asset dry negatives without creating state.
**Output**: Root policy/event, App, and governance-observer identities; authenticated viewer/ADMIN and exact GET route; two complete raw-private response-header locators; canonical signed one-use snapshot bytes/digest/nonce/expiry; normalized tag ruleset/settings/bypass/digest/ETag before/after; broker code/policy/key fingerprint and route inventory; exact future tag name/object and body-only Release digests/absent-ref/empty-assets proof; negative principal/method receipts; and no-mutation proof.
**Acceptance**: The two no-cache privileged observations include full bypass actors and identical nonempty ETag plus normalized digest; snapshot signature, signer generation, nonce, expiry, viewer, and exact nested-name match validate. Provider settings and broker policy permit only one canonical absent-ref creation by Ledger Writer for the exact future tag/body-only empty-assets Release and deny every other creator plus all update/delete/force/upload routes. Missing/omitted bypass data, absent/unstable version indicator, inactive/drifting rule, extra/missing bypass, collision, route overreach, or identity mismatch blocks Unit 66; no tag/Release/provider mutation occurs.

### ⬜ Unit 66: Protected Tag and GitHub Release Projection
**What**: Only after Units 65c-65d succeed for the current observed/effective `ReleaseSetPublished`, authorize and claim `release-tag-project-v1` bound to Unit 65d's fresh signed unconsumed `ProtectedReleaseTagSnapshotV1`, deterministic `release-set/<ProductChangeID>/<publicationSHA>` name, exact canonical annotated-tag object bytes/OID, absent-ref prestate, publication/observation/main-pointer/attestation digests, and exact body-only GitHub Release request. The Release body contains immutable sanitized commit/attestation links and digests; `assets=[]` is mandatory and no upload endpoint exists. The Ledger Writer broker verifies/atomically consumes the pre-snapshot nonce, re-queries ref/release, mints a bounded token, creates the annotated tag object and ref with create-if-absent only, then creates the exact body-only Release. Lost responses re-query exact tag/ref/object plus Release name/body/target/draft/prerelease/asset inventory; exact equality with empty assets is idempotent success, while any mismatch/collision/nonempty asset set contains. Enter `AWAITING_RULESET_POST_OBSERVATION`, revoke or contain the write token, and accept only a fresh separately nonced governance-observer post-snapshot from the same exact no-cache GET with full bypass actors, identical ETag and normalized digest, current signer generation, and valid expiry. Consume that nonce, re-query exact state, and append terminal. No update/delete/force/upload API exists.
**Output**: Authorization/claim/awaiting-post/terminal-or-containment commits; run/attempt/broker/token identities; signed pre/post snapshot bytes/digests/nonces/expiries and identical ruleset ETag/normalized digests; canonical tag object/ref/name/OID and create receipt; collision/retry reconciliation; exact body-only GitHub Release fields/links/digests, empty asset inventory, IDs and URL; token revocation-or-expiry containment; update/delete/upload route-absence proof; and authoritative post-query.
**Acceptance**: Guard rejects absent/aborted/stale generations, wrong namespace/object/principal, missing/omitted bypass actors, stale/replayed/cross-signed snapshot, missing/changing ETag or normalized digest, permissive/drifting rules, existing mismatch, nonempty/duplicate assets, and update/delete/force/upload. Exactly Ledger Writer creates the absent canonical nested tag once under active `ProtectedReleaseTagV1`; retries prove exact equality; the body-only empty-assets GitHub Release identifies the current Unit 64c publication and Unit 65c pointer; matching fresh privileged pre/post snapshots and terminal token handling complete before terminal; no private artifact and no alternate shipment authority.

...
**What**: Add red semantic tests for authority roles, GitHub actor/run provenance, dedicated ledger GitHub App/installation identity, transitions/generations/CAS, supervisor-envelope provisional start/challenge/provision/abort/rotation/retirement with distinct repository identities and active-run retention, durable recovery registration/expiry, resolver gating across reservation/claim/pending publication, coordinator/recovery races, direct-child finalization plus mandatory observation effectiveness proof, unauthorized App or ordinary `GITHUB_TOKEN` appends, receiver/bootstrap/drift recovery, and every ordering.
...
**What**: Add SHA-pinned CI/resolver. Define `ProtectedMainV1` with repository-specific `{context, expected App ID}` checks, and `ProtectedLedgerV1` with exactly the policy-pinned ledger App bypass and no ordinary writer path; both normalize/stability-check all layers. Implement serialized broker identity/actor/transition validation plus distinct web/native envelope-key generations and one-use unwrap. Capsule encryption is RFC 9180 base-mode HPKE suite `{KEM=X25519-HKDF-SHA256(0x0020),KDF=HKDF-SHA256(0x0001),AEAD=ChaCha20Poly1305(0x0003)}` with a fresh ephemeral KEM key and sequence number zero per capsule. Strict RFC 8785 envelope is exactly `{version:1,suite,enc,ciphertext,payloadDigest,context}` with 32-byte `enc` and base64url-no-padding fields. `info` is `UTF8("spoonjoy-supervisor-capsule-v1") || 0x00 || RFC8785({repository,workflow,keyGeneration})`; AAD is RFC 8785 context binding owner/repository/workflow/ref/SHA/run/attempt/checkRun/job/environment/claim/generation/nonce/expiry/payloadDigest/signerPublicKey. `payloadDigest` hashes canonical plaintext; the claim separately binds SHA-256 of the final envelope bytes. Reject reused `enc`/nonce/context and all noncanonical encodings. Normal unwrap requires exact active provisioned key event and validates immutable GitHub OIDC claims plus JTI, consumes a Durable Object nonce before decrypt, returns signer bytes only once, and rejects replay/retired/contained generations. A separate provisional-challenge endpoint requires exact `ProvisioningStarted` event/claim/generation/challenge job/nonce, decrypts only the fixed disposable challenge payload once, and cannot unwrap signer capsules; success alone authorizes `Provisioned`, while timeout/crash deletes the binding and appends `ProvisioningAborted` or contains all unwraps. App and envelope private keys never leave broker storage. Model append state independently as `prepared -> committing -> cas_succeeded|cas_uncertain -> observed|abort_authorized -> terminal`, and each credential lease as `none -> token_mint_started -> token_issued -> token_revoke_started -> token_revoked|token_contained`. Before every write-token POST, persist attempt/authorized transition, no-cache GitHub Date, 15000ms mint deadline, and `containUntil = Date + 3675000ms`; lost response globally blocks writes. Persist returned expiry/HMAC fingerprint before Git work. Each write token is memory-only and revoked in `finally` with required `204`, else retry or contain until expiry. `H` has no write token. Exact unchanged-head evidence authorizes abort CAS only as a new journaled write-token attempt. Preserve `cas_uncertain` across teardown/restarts.
...
**What**: Authorize and claim one `web-supervisor-envelope-key-provision-v1` operation under the root policy's fixed repository/workflow/key-lifecycle graph. Generate a distinct X25519 keypair inside an enumerable private context before provider mutation; derive public key/fingerprint, then broker-append `SupervisorEnvelopeKeyProvisioningStarted` binding claim, next generation, exact public key/fingerprint, expected web challenge workflow/job, one nonce, and expiry. Through a one-use scoped Cloudflare credential stream the matching private key into a provisional broker binding. Dispatch/approve only the bound disposable web challenge job; the provisional endpoint requires exact start event/OIDC/claim/generation/nonce and decrypts only the fixed test payload once. After success, broker-appends `SupervisorEnvelopeKeyProvisioned` and activates ordinary unwrap. Crash before start only purges local context; timeout/crash/failure after start deletes binding and appends `ProvisioningAborted`; uncertain deletion contains all unwraps. Inventory generation, destroy/fsync local plaintext, revoke bootstrap credential, and journal every partial failure. Rotation provisions N+1 provisionally, keeps N decrypt-only until bound runs terminate, then retires/deletes N.
**Output**: Authorization/claim/start/provision-or-abort/retire/terminal-or-containment commits; public-key event/policy digests; provisional/active binding generation/fingerprint; scoped credential creation/revocation; exact challenge; local-zero/key inventory; rotation/deletion/partial-failure receipts; and post-query.
...
**What**: Authorize and claim one `native-supervisor-envelope-key-provision-v1` operation under the root policy's fixed repository/workflow/key-lifecycle graph. Generate a distinct X25519 keypair inside an enumerable private context before provider mutation; derive public key/fingerprint, then broker-append `SupervisorEnvelopeKeyProvisioningStarted` binding claim, next generation, exact public key/fingerprint, expected native challenge workflow/job, one nonce, and expiry. Through a separate one-use scoped Cloudflare credential stream the matching private key into a provisional broker binding. Dispatch/approve only the bound disposable native challenge job; the provisional endpoint requires exact start event/OIDC/claim/generation/nonce and decrypts only the fixed test payload once. After success, broker-appends `SupervisorEnvelopeKeyProvisioned` and activates ordinary unwrap. Crash before start only purges local context; timeout/crash/failure after start deletes binding and appends `ProvisioningAborted`; uncertain deletion contains all unwraps. Inventory generation, destroy/fsync local plaintext, revoke bootstrap credential, and journal every partial failure. Rotation provisions N+1 provisionally, keeps N decrypt-only until bound runs terminate, then retires/deletes N.
**Output**: Authorization/claim/start/provision-or-abort/retire/terminal-or-containment commits; public-key event/policy digests; provisional/active binding generation/fingerprint; scoped credential creation/revocation; exact challenge; local-zero/key inventory; rotation/deletion/partial-failure receipts; and post-query.

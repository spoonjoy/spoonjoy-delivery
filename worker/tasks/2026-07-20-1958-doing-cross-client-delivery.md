...
**What**: Split production workflow into preflight/claimed operations and make Storybook push/PR build-only. Pin Node to `22.22.0`. Credential-free preflight independently re-queries the protected ledger ref/event/protection digest, validates the policy-authorized active web envelope transition, fetches the broker's public key, and requires exact generation/fingerprint/key/event-commit equality before encrypting. It builds and attests the portable Node/Wrangler/static artifact, exact supervisor binary/policy, an ephemeral Ed25519 teardown public key, and private key encrypted to that verified envelope key; plaintext key is destroyed before upload. Bind envelope-key event/protection, artifact/supervisor/policy/public-key/capsule/run/nonce digests into the Execution Claim before approval. Protected `deployment:false` job presents its exact GitHub OIDC identity and claim to the broker for one-use capsule unwrap directly into the out-of-child supervisor's memory, verifies the whole artifact, and invokes bundled Node/Wrangler in the Linux sandbox. The unwrap key never enters Actions; no setup/install/package network or child access to signer/unwrap material exists. Emit no GitHub Deployment/status and produce the pre-bound teardown proof.
...
**What**: Authorize and claim one `web-supervisor-envelope-key-provision-v1` operation under the root policy's fixed repository/workflow/key-lifecycle graph. Broker-append `SupervisorEnvelopeKeyProvisioningStarted` binding claim, next generation, expected web challenge workflow/job, one nonce, expiry, public-key target, and binding fingerprint. Through a one-use scoped Cloudflare credential, generate a distinct X25519 keypair inside an enumerable private context and stream the private key into a provisional broker binding. Dispatch/approve only the bound disposable web challenge job; the provisional endpoint requires the exact start event/OIDC/claim/generation/nonce and decrypts only the fixed test payload once. After success, broker-appends `SupervisorEnvelopeKeyProvisioned` carrying generation/public key/fingerprint/challenge/policy digests and activates ordinary unwrap. Timeout/crash/failure deletes the binding and appends `SupervisorEnvelopeKeyProvisioningAborted`; uncertain deletion contains all unwraps. Inventory generation, destroy/fsync local plaintext, revoke the bootstrap credential, and journal every partial failure. Rotation provisions N+1 provisionally, keeps N decrypt-only until bound runs terminate, then appends retirement and deletes N.
...
**What**: Authorize and claim one `native-supervisor-envelope-key-provision-v1` operation under the root policy's fixed repository/workflow/key-lifecycle graph. Broker-append `SupervisorEnvelopeKeyProvisioningStarted` binding claim, next generation, expected native challenge workflow/job, one nonce, expiry, public-key target, and binding fingerprint. Through a separate one-use scoped Cloudflare credential, generate a distinct X25519 keypair inside an enumerable private context and stream the private key into a provisional broker binding. Dispatch/approve only the bound disposable native challenge job; the provisional endpoint requires the exact start event/OIDC/claim/generation/nonce and decrypts only the fixed test payload once. After success, broker-appends `SupervisorEnvelopeKeyProvisioned` carrying generation/public key/fingerprint/challenge/policy digests and activates ordinary unwrap. Timeout/crash/failure deletes the binding and appends `SupervisorEnvelopeKeyProvisioningAborted`; uncertain deletion contains all unwraps. Inventory generation, destroy/fsync local plaintext, revoke the bootstrap credential, and journal every partial failure. Rotation provisions N+1 provisionally, keeps N decrypt-only until bound runs terminate, then appends retirement and deletes N.
**Output**: Authorization/claim/start/provision-or-abort/retire/terminal-or-containment commits; public-key event/policy digests; provisional/active binding generation/fingerprint; scoped credential creation/revocation; exact challenge; local-zero/key inventory; rotation/deletion/partial-failure receipts; and post-query.

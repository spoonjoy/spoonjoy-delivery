...
- Before Unit 67 emits the completed execution stop, the root coordinating host must fsync and register the exact private `RootTaskPostStopCleanupCursorV1` in Desk. Session start/resumption must consume that cursor one action at a time after the execution stop, prove terminal-ledger bytes unchanged, and finish its receipt before any final human completion message. The cursor may remove only its manifest-bound current local delivery worktree/branch and private cleanup state; it grants no source/provider read or mutation authority.
